...
ควรคำนวณที่ฝั่ง Kestrel Server เพราะช่วยแยกหน้าที่ระหว่างอุปกรณ์และเซิร์ฟเวอร์ โดยให้ ESP32 ทำหน้าที่อ่านค่า Raw จากเซนเซอร์และส่งข้อมูลดิบไปยังเซิร์ฟเวอร์ ส่วน Kestrel Server ทำหน้าที่คำนวณและแปลงค่าให้อยู่ในหน่วยที่ต้องการ เช่น อุณหภูมิหรือความชื้น ทำให้สามารถปรับสูตรการคำนวณหรือสเกลได้ง่ายโดยไม่ต้องแก้ไขและอัปโหลด Firmware ใหม่ให้กับ ESP32
...
หาก RawMax <= RawMin จะทำให้ช่วงของค่าที่ใช้ในการคำนวณสเกลไม่ถูกต้อง และหากเกิดการหารด้วยศูนย์ในกรณี RawMax == RawMin จะเกิด DivideByZeroException ในภาษา C# ส่งผลให้การประมวลผลคำขอ (Request) นั้นเกิดข้อผิดพลาด และหากไม่มีการจัดการ Exception อย่างเหมาะสม อาจทำให้ API ตอบกลับด้วยข้อผิดพลาดแทนข้อมูลที่ถูกต้อง
...
เนื่องจาก HTTP POST ที่ส่งข้อมูล JSON ต้องระบุ Header Content-Type: application/json เพื่อแจ้งให้ Kestrel Server ทราบว่า ข้อมูลใน Request Body มีรูปแบบเป็น JSON หากไม่มี Header นี้ เซิร์ฟเวอร์จะไม่สามารถระบุหรือไม่ยอมรับรูปแบบข้อมูลที่ส่งมาเป็น JSON ตามที่ API กำหนด จึงปฏิเสธคำขอและส่งรหัสสถานะ 415 Unsupported Media Type กลับไป
