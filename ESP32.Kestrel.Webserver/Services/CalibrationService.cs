...
public class CalibrationService
{
    ...
    public void UpdateSettings(CalibrationSettings newSettings)
    {
        if (newSettings.RawMax <= newSettings.RawMin)
        {
            throw new ArgumentException(
                "RawMax ต้องมีค่ามากกว่า RawMin เสมอ!"
            );
        }
    ...
    {
        _currentOledMessage = msg.Length > 20
            ? msg[..20]
            : msg;
    }
    ...
    public double Compute(int rawAdc)
    {
        ...
        return ((double)(clamped - _settings.RawMin)
                / (_settings.RawMax - _settings.RawMin))
                * (_settings.ScaleMax - _settings.ScaleMin)
                + _settings.ScaleMin;
    }
}