/*
 * ==========================================================================
 *  Lab 9.3 : Closed-Loop IoT Integration (Full-Duplex + Hybrid Edge-Cloud)
 *  Target  : ESP32 + SSD1306 SPI OLED + Potentiometer 10k (ADC1 GPIO 34)
 * --------------------------------------------------------------------------
 *  โปรโตคอลบนสาย Serial (115200 8N1) — '\n' คือ Packet Delimiter เสมอ
 *    ขาขึ้น (ESP32 -> Kestrel) : "ADC:<raw>,<uptime_ms>\n"
 *    ขาลง   (Kestrel -> ESP32) : "SET:<percent>:<message>\n"
 *
 *  หมายเหตุการแก้ไขจากโค้ดต้นฉบับในใบงาน 9.3:
 *    - เปลี่ยน "ID: 65012345" เป็นรหัสนักศึกษาจริง 67030098
 *    - main/CMakeLists.txt แก้ SRCS ให้ตรงกับชื่อไฟล์จริง (main.c)
 *  ตรรกะและโครงสร้างฟังก์ชันอื่นทั้งหมดคงไว้ตามที่ใบงานกำหนดทุกประการ
 * ==========================================================================
 */
...
void render_multizone_ui(int raw_val, int percent, bool is_cloud, const char *msg) {
    ...
    char header_str[32];
    snprintf(header_str, sizeof(header_str), "ESP32 | 67030098"); // รหัสนักศึกษาจริง
