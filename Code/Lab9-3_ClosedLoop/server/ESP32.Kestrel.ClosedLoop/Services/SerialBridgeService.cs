...
/// <summary>
/// Two-Way Serial Bridge ตามใบงาน 9.3 (ขั้นที่ 3.2.3)
///   ขาขึ้น (ESP32 -> Kestrel) : "ADC:<raw>,<uptime_ms>\n"
///   ขาลง   (Kestrel -> ESP32) : "SET:<percent>:<message>\n"
///
/// ทำงานเป็น BackgroundService วนอ่าน/เขียนตลอดอายุของแอป และ auto-reconnect
/// ทุก 2 วินาทีเมื่อพอร์ตหลุดหรือไม่มีบอร์ดต่ออยู่ — ตรงตามที่ใบงานออกแบบไว้
/// </summary>
public class SerialBridgeService : BackgroundService
{
    ...
    {
        // อ่านค่า COM Port จาก appsettings.json (คีย์ SerialPort:PortName) หรือ fallback
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // หมดเวลารอข้อมูลรอบปกติ (ไม่มีบอร์ดต่ออยู่ หรือบอร์ดยังไม่ส่งข้อมูล) ให้วนรอบต่อไป
            }
            catch (Exception ex)
            {
