...
คำตอบ:
ตัวอักษร 'H' ใช้ข้อมูลจำนวน 5 ไบต์ เพราะรูปแบบตัวอักษรใช้โครงสร้างพิกเซลแบบ 5 คอลัมน์ × 7 แถว โดยแต่ละไบต์ใช้แทนข้อมูลของ 1 คอลัมน์ในแนวตั้ง ของตัวอักษร ดังนั้นข้อมูลทั้ง 5 ไบต์จึงควบคุมพิกเซลในทิศทาง บนลงล่างของแต่ละคอลัมน์ และเมื่อนำทั้ง 5 คอลัมน์มาเรียงต่อกันจาก ซ้ายไปขวา จะเกิดเป็นรูปร่างของตัวอักษร 'H'

2. หากเราสลับสายไฟระหว่างขา **D0** และ **D1** จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?
คำตอบ:
หากสลับสาย D0 และ D1 ซึ่งเป็นสายสัญญาณ SCK (Clock) และ MOSI (Data) ของ SPI สัญญาณนาฬิกาและข้อมูลจะถูกส่งไปผิดขา ทำให้ OLED ไม่สามารถตีความข้อมูล SPI ได้อย่างถูกต้อง ส่งผลให้หน้าจอ ไม่สามารถแสดงผลข้อมูลได้ตามปกติ เช่น จอไม่แสดงผลหรือแสดงภาพผิดปกติ แม้ ESP32 จะยังทำงานและส่งข้อมูลออกมาอยู่ก็ตาม

3. เหตุใดการแก้ไขพิกัด $(x, y)$ บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
คำตอบ:
เนื่องจาก s_oled_buffer เป็น หน่วยความจำสำรอง (Framebuffer) ที่เก็บข้อมูลภาพไว้ใน RAM ของ ESP32 การแก้ไขพิกัด (x, y) จะเปลี่ยนเฉพาะข้อมูลที่อยู่ใน Buffer แต่ยังไม่ได้ส่งข้อมูลนั้นไปยังจอ OLED จริง ดังนั้นจึงต้องเรียก oled_flush() เพื่อส่งข้อมูลทั้งหมดจาก s_oled_buffer ผ่าน SPI ไปยังหน่วยความจำของ OLED เมื่อส่งข้อมูลสำเร็จ ภาพบนหน้าจอจริงจึงเปลี่ยนตามข้อมูลใหม่