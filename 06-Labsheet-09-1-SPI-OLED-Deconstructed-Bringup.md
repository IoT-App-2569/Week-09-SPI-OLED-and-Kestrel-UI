...
|**Bit (Row)**|**Col 0 (0x7F)**|**Col 1 (0x08)**|**Col 2 (0x08)**|**Col 3 (0x08)**|**Col 4 (0x7F)**|**Col 5 (0x00 ช่องไฟ)**|
|---|---|---|---|---|---|---|
|**Bit 0** (แถวบนสุด)|**1** (■)|0 (□)|0 (□)|0 (□)|**1** (■)|0 (□)|
|**Bit 1**|**1** (■)|0 (□)|0 (□)|0 (□)|**1** (■)|0 (□)|
|**Bit 2**|**1** (■)|0 (□)|0 (□)|0 (□)|**1** (■)|0 (□)|
|**Bit 3** (แกนกลาง)|**1** (■)|**1** (■)|**1** (■)|**1** (■)|**1** (■)|0 (□)|
|**Bit 4**|**1** (■)|0 (□)|0 (□)|0 (□)|**1** (■)|0 (□)|
|**Bit 5**|**1** (■)|0 (□)|0 (□)|0 (□)|**1** (■)|0 (□)|
|**Bit 6** (แถวล่างสุด)|**1** (■)|0 (□)|0 (□)|0 (□)|**1** (■)|0 (□)|
|**Bit 7** (ไม่ได้ใช้งาน)|0 (□)|0 (□)|0 (□)|0 (□)|0 (□)|0 (□)|
...
ตอบ - **เหตุผลที่ใช้ 5 ไบต์:** มาจากขนาดของตารางฟอนต์ 5x7 พิกเซล (กว้าง 5 คอลัมน์ สูง 7 พิกเซล) โดยข้อมูล 1 คอลัมน์จะถูกเก็บด้วยข้อมูลขนาด 1 ไบต์ จึงต้องใช้ 5 ไบต์เพื่อประกอบเป็นความกว้างของตัวอักษร

**ทิศทางการควบคุม:** ควบคุมพิกเซลใน **แนวตั้ง (Vertical)** โดย 8 บิตภายใน 1 ไบต์ (Bit 0 ถึง Bit 7) จะทำหน้าที่คุมจุดพิกเซลเรียงจากบนลงล่างในแต่ละคอลัมน์
2. หากเราสลับสายไฟระหว่างขา **D0** และ **D1** จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?
ตอบ - **ผลต่อสัญญาณ SPI:** ระบบบัสจะทำงานล้มเหลวทันที เพราะสัญญาณนาฬิกา (Clock) และข้อมูล (Data) สลับหน้าที่กัน ทำให้ชิป SSD1306 จับจังหวะบิต (Synchronization) ผิดพลาดและถอดรหัสคำสั่งไม่ได้
    
- **การติดของหน้าจอ:** **หน้าจอจะไม่ติด (ดับสนิท)** เนื่องจากไม่สามารถรับคำสั่งเปิดวงจรทวีแรงดัน (Charge Pump `0x8D, 0x14`) และคำสั่งเปิดหน้าจอ (`0xAF`) ได้สำเร็จ




3. เหตุใดการแก้ไขพิกัด $(x, y)$ บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
ตอบ - ตัวแปร `s_oled_buffer` เป็นเพียงหน่วยความจำสำรอง (Back Buffer) ที่อยู่ใน **RAM ของ ESP32** เท่านั้น ไม่ใช่แรมภายในตัวจอแสดงผล
    
- พิกเซลบนจอจริงจะเปลี่ยนก็ต่อเมื่อมีการคัดลอกข้อมูล 1,024 ไบต์จาก RAM ของ ESP32 ผ่านบัส SPI ไปเขียนทับลงบนแรมภาพ (GDDRAM) ภายในชิป SSD1306 ซึ่งคำสั่ง `oled_flush()` เป็นตัวทำหน้าที่ส่งข้อมูลชุดนี้