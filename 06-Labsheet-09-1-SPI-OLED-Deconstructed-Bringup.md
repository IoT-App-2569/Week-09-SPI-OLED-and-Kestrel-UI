...
เพราะตาราง font5x7 ออกแบบให้ตัวอักษรแต่ละ   ตัวมีความกว้าง 5 พิกเซล (5 คอลัมน์) โดยแต่ละไบต์ในอาร์เรย์ font5x7[font_idx][col] แทนข้อมูล 1 คอลัมน์แนวตั้ง ของตัวอักษรนั้น ภายในไบต์เดียวนี้ แต่ละบิต (บิต 0 ถึงบิต 6 รวม 7 บิต จากความสูงฟอนต์ 7 พิกเซล) ควบคุมพิกเซลในทิศทางแนวตั้ง (row) ของคอลัมน์นั้น กล่าวคือ ไบต์ทำหน้าที่เป็น "แท่งพิกเซลแนวตั้งบาง ๆ กว้าง 1 คอลัมน์" และเมื่อนำ 5 ไบต์มาเรียงต่อกันตามลำดับคอลัมน์ จะประกอบขึ้นเป็นรูปทรงตัวอักษรที่สมบูรณ์ บิตที่ 7 (MSB) มักไม่ได้ใช้งาน (ปล่อยเป็น 0) เพราะฟอนต์สูงเพียง 7 พิกเซล ไม่ใช่ 8

2. หากเราสลับสายไฟระหว่างขา D0 และ D1 จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?

ขา D0 คือ SCK (SPI Clock) และขา D1 คือ MOSI (Data) หากสลับสายกัน จะทำให้สัญญาณนาฬิกาที่ควรไปกระตุ้นขา SCK ของชิปไปออกที่ขา MOSI แทน และข้อมูลบิตที่ควรส่งทาง MOSI กลับไปออกที่ขา SCK แทน ผลคือ SSD1306 จะสุ่มตัวอย่างข้อมูล (sample) ผิดจังหวะ เพราะโปรโตคอล SPI อาศัยการซิงก์ระหว่างขอบสัญญาณนาฬิกา (clock edge) กับสถานะของขา Data ที่ตำแหน่งคงที่ เมื่อสลับขากัน ชิปจะตีความค่าที่รับมาเป็นค่าขยะ (garbage) ไม่ตรงกับคำสั่งหรือข้อมูลพิกเซลที่ต้องการส่งจริง หน้าจอจะไม่ติดเลย หรือแสดงผลเป็น Noise/Garbage แบบสุ่ม ไม่สามารถแสดงภาพที่ถูกต้องได้ และอาจไม่รับคำสั่ง Charge Pump/Display ON อย่างถูกต้องด้วย ทำให้จอมืดสนิท

3. เหตุใดการแก้ไขพิกัด (x, y) บน s_oled_buffer จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง oled_flush()?

เพราะ s_oled_buffer เป็นเพียง Back Buffer ที่อยู่ในหน่วยความจำ RAM ของ ESP32 เท่านั้น ไม่ได้เชื่อมโยงโดยตรงกับหน่วยความจำแสดงผล (GDDRAM) ภายในชิป SSD1306 ฟังก์ชัน oled_draw_pixel() ทำหน้าที่แค่คำนวณและแก้ไขค่าบิตในอาร์เรย์นี้ ซึ่งเป็นการแก้ไข "ข้อมูลในกระดาษร่าง" ที่ยังไม่ถูกส่งออกไปไหน ส่วนฟังก์ชัน oled_flush() เป็นขั้นตอนเดียวที่ทำการยิงข้อมูลทั้ง 1,024 ไบต์ผ่านบัส SPI ไปเขียนทับ GDDRAM จริงภายในชิป OLED ดังนั้นจอภาพจริงจะอัปเดตก็ต่อเมื่อมีการเรียก oled_flush() เท่านั้น สถาปัตยกรรมแบบ Double Buffering นี้มีข้อดีคือช่วยให้นักศึกษาสามารถวาดพิกเซลหลาย ๆ จุด/ตัวอักษรหลายตัวเสร็จสมบูรณ์ในหน่วยความจำก่อน แล้วค่อยส่งขึ้นจอทีเดียว ทำให้ภาพไม่กระพริบระหว่างการวาดทีละพิกเซล (Tearing/Flicker)