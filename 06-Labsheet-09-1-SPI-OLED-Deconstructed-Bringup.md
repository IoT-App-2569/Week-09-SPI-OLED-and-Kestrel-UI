...
<img width="3024" height="4032" alt="IMG_4482" src="https://github.com/user-attachments/assets/7a30b118-e646-4632-9b2d-5464b4fd8542" />
...
<img width="4032" height="3024" alt="IMG_4483" src="https://github.com/user-attachments/assets/892b901d-4deb-41a7-b506-fda2eed68193" />
...
<img width="3024" height="4032" alt="IMG_4485" src="https://github.com/user-attachments/assets/c9100a23-5cd1-40fd-9215-5915eea84081" />
...
>
```
ภาพกลับหัวเพราะในชุดคำสั่งเริ่มต้นเราไม่ได้สั่งทิศทางการสแกนของจอเลย ชิป SSD1306 จึงใช้ค่าเริ่มต้นของมันเอง ซึ่งกับโมดูลจอรุ่นนี้ที่เดินสายแผงมาแบบกลับด้าน ผลคือแถวแรกของข้อมูลไปโผล่ที่ด้านล่างของจอ โค้ดวาดพิกัด 0,0 ที่มุมบนซ้ายถูกต้องแล้ว ปัญหาอยู่ที่ฮาร์ดแวร์สแกนสลับทิศ ทิศทางนี้ถูกกำหนดด้วยคำสั่ง 0xA0 หรือ 0xA1 ที่กลับซ้ายขวา และ 0xC0 หรือ 0xC8 ที่กลับบนล่าง
```
...
| คอลัมน์ | ไบต์ | เพจ 0 (y=0-7) | บิตที่เป็น 1 ในเพจ 0 | เพจ 1 (y=8-15) | บิตที่เป็น 1 ในเพจ 1 | แถว y ที่ติด |
|---|---|---|---|---|---|---|
| 30 | 0xF0, 0x07 | 11110000 | 4, 5, 6, 7 | 00000111 | 0, 1, 2 | 4 ถึง 10 |
| 31 | 0x80, 0x00 | 10000000 | 7 | 00000000 | ไม่มี | 7 |
| 32 | 0x80, 0x00 | 10000000 | 7 | 00000000 | ไม่มี | 7 |
| 33 | 0x80, 0x00 | 10000000 | 7 | 00000000 | ไม่มี | 7 |
| 34 | 0xF0, 0x07 | 11110000 | 4, 5, 6, 7 | 00000111 | 0, 1, 2 | 4 ถึง 10 |
| 35 | 0x00, 0x00 | 00000000 | ไม่มี | 00000000 | ไม่มี | ไม่มี (ช่องไฟ) |

พอเอาบิตที่เป็น 1 มาวางเป็นกริด แนวนอนคือคอลัมน์ 30 ถึง 34 และแนวตั้งคือแถว y=4 ถึง 10 จะได้รูปนี้ 
...
ข้อมูลที่ตัวอักษรคร่อมสองเพจก็ตรงกับที่คาด เพราะวาดที่ y=4 ขีดตั้งจึงใช้บิต 4 ถึง 7 ของเพจ 0 (4 แถว) ต่อกับบิต 0 ถึง 2 ของเพจ 1 (3 แถว) รวม 7 แถว
ตรงกับบนจอจริง
---
...
```
H ใช้ 5 ไบต์เพราะฟอนต์กว้าง 5 คอลัมน์ หนึ่งไบต์คือหนึ่งคอลัมน์ และแต่ละบิตคุมพิกเซลแนวตั้ง บิต 0 อยู่บนสุดของเพจ จากผล dump คอลัมน์ 30 กับ 34 ได้ 0xF0 กับ 0x07 คือขีดตั้งสูง 7 แถว ส่วนคอลัมน์ 31 ถึง 33 ได้ 0x80 คือขีดขวางแถวเดียว
```
2. หากเราสลับสายไฟระหว่างขา **D0** และ **D1** จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?
```
clock กับ ข้อมูลจะสลับกัน ชิปอ่านคำสั่งไม่ได้ จอจึงไม่ติดหรือขึ้นภาพมั่ว ส่วนโค้ดฝั่ง ESP32 ยังรันปกติ ไม่ฟ้อง error เพราะจอเขียนได้อย่างเดียว
```
3. เหตุใดการแก้ไขพิกัด $(x, y)$ บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
```
เพราะ s_oled_buffer เป็นแค่สำเนาในแรมของ ESP32 จอมีหน่วยความจำของตัวเองในชิป ภาพจะเปลี่ยนก็ต่อเมื่อ oled_flush ส่งข้อมูลผ่าน SPI ไปเขียนทับ
```
