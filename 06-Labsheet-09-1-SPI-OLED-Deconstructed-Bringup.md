...
#### บันทึกผลการทดลอง: ค่าที่อ่านได้จาก Serial Monitor
```text
I (xxxx) FORENSIC: === DUMPING FRAMEBUFFER PAGE 0 (First 16 Bytes) ===
...
#### ตารางถอดรหัสบิตของตัวอักษร 'H'

| คอลัมน์ | ไบต์ที่ | ค่า Hex | ค่า Binary | บิตที่เป็น 1 | ส่วนประกอบของตัวอักษร |
| :---: | :---: | :---: | :---: | :---: | :--- |
| **Col 0** | Byte[0] | `0x7F` | `01111111` | Bit 0 ถึง 6 | ขีดแนวตั้งด้านซ้าย |
| **Col 1** | Byte[1] | `0x08` | `00001000` | Bit 3 | ขีดแนวนอนตรงกลาง |
| **Col 2** | Byte[2] | `0x08` | `00001000` | Bit 3 | ขีดแนวนอนตรงกลาง |
| **Col 3** | Byte[3] | `0x08` | `00001000` | Bit 3 | ขีดแนวนอนตรงกลาง |
| **Col 4** | Byte[4] | `0x7F` | `01111111` | Bit 0 ถึง 6 | ขีดแนวตั้งด้านขวา |
| **Col 5** | Byte[5] | `0x00` | `00000000` | ไม่มี | ช่องว่างเว้นระหว่างตัวอักษร |

#### ตารางแสดงตำแหน่งพิกเซล (นำค่าบิตมากระจายตัวตามแถว Row 0 - 7)
*(หมายเหตุ: บนจอ SSD1306 บิต 0 คือแถวบนสุด และบิต 7 คือแถวล่างสุด)*

| แถว (Row) | Col 0 | Col 1 | Col 2 | Col 3 | Col 4 | Col 5 | ภาพบนจอ (1 = จุดสว่าง, 0 = ดับ) |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Row 0 (Bit 0)** | **1** | 0 | 0 | 0 | **1** | 0 | `1 . . . 1 .` |
| **Row 1 (Bit 1)** | **1** | 0 | 0 | 0 | **1** | 0 | `1 . . . 1 .` |
| **Row 2 (Bit 2)** | **1** | 0 | 0 | 0 | **1** | 0 | `1 . . . 1 .` |
| **Row 3 (Bit 3)** | **1** | **1** | **1** | **1** | **1** | 0 | `1 1 1 1 1 .` |
| **Row 4 (Bit 4)** | **1** | 0 | 0 | 0 | **1** | 0 | `1 . . . 1 .` |
| **Row 5 (Bit 5)** | **1** | 0 | 0 | 0 | **1** | 0 | `1 . . . 1 .` |
| **Row 6 (Bit 6)** | **1** | 0 | 0 | 0 | **1** | 0 | `1 . . . 1 .` |
| **Row 7 (Bit 7)** | 0 | 0 | 0 | 0 | 0 | 0 | `. . . . . .` |

#### วิเคราะห์และสรุปผล
1. **เส้นแนวตั้งซ้าย-ขวา:** ที่ Col 0 และ Col 4 มีค่า `0x7F` ซึ่งแปลงเป็นฐานสองคือ `01111111` จะเห็นว่าบิต 0 ถึง 6 เป็นเลข 1 ทั้งหมด ทำให้พิกเซลติดสว่างเป็นเส้นแนวตั้งทั้งสองฝั่ง
2. **เส้นแนวนอนตรงกลาง:** ที่ Col 1, 2 และ 3 มีค่า `0x08` หรือ `00001000` มีแค่บิต 3 ตัวเดียวที่เป็นเลข 1 จึงเป็นจุดที่เชื่อมเสาซ้ายกับเสาขวาเข้าด้วยกันตรงกลาง
3. **ช่องไฟ:** แถวที่ 7 เป็นเลข 0 ทั้งหมดเพื่อเว้นบรรทัดด้านล่าง และ Col 5 เป็นเลข 0 ทั้งหมด (`0x00`) เพื่อเว้นช่องไฟระหว่างตัวอักษร
- **สรุป:** เมื่อนำค่าบิต 1 ในหน่วยความจำแรม `s_oled_buffer` มาพล็อตดู จะได้รูปร่างตรงกับตัวอักษร **'H'** ที่แสดงบนจอ OLED จริง
...
1. จากการทำ Hex Dump ในกิจกรรมนิติวิทยาศาสตร์ จงอธิบายว่าทำไมตัวอักษร `'H'` จึงใช้ข้อมูลจำนวน 5 ไบต์ และแต่ละไบต์ทำหน้าที่ควบคุมพิกเซลในทิศทางใด?
   - **ตอบ:** เพราะฟอนต์ที่ใช้เป็นขนาด 5x7 พิกเซลกว้าง 5 คอลัมน์ สูง 7 แถว และโครงสร้างแรมของจอ SSD1306 จะเก็บข้อมูลแบบ 1 ไบต์ต่อ 1 คอลัมน์ในแนวตั้ง (บิต 0 ถึง บิต 7) ดังนั้นตัวอักษรที่มีความกว้าง 5 คอลัมน์ จึงต้องใช้ข้อมูล 5 ไบต์เรียงกันในแนวนอน โดยแต่ละไบต์จะควบคุมพิกเซลใน**แนวตั้ง**ของคอลัมน์นั้นๆ
2. หากเราสลับสายไฟระหว่างขา **D0** และ **D1** จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?
   - **ตอบ:** หน้าจอจะไม่ติด เพราะขา D0 คือสัญญาณนาฬิกา (SCK) ส่วน D1 คือสายส่งข้อมูล (MOSI) ถ้าสลับสายกัน ตัวจอจะไม่มีสัญญาณ Clock มาคอยจับจังหวะอ่านข้อมูล ทำให้ชิป SSD1306 อ่านคำสั่งผิดพลาดและไม่สามารถสั่งเปิดวงจร Charge Pump หรือเปิดจอได้ หน้าจอจึงดับสนิท
3. เหตุใดการแก้ไขพิกัด $(x, y)$ บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
   - **ตอบ:** เพราะตัวแปร `s_oled_buffer` เป็น Framebuffer ที่อยู่ในแรมของบอร์ด ESP32 เท่านั้น ฟังก์ชันพวก `oled_draw_pixel()` เป็นแค่การเปลี่ยนค่าในแรมของ ESP32 ยังไม่ได้ส่งไปที่จอภาพ ต้องเรียกคำสั่ง `oled_flush()` ก่อน ตัว ESP32 ถึงจะส่งข้อมูลทั้งหมด 1,024 ไบต์ผ่านบัส SPI ไปอัปเดตลงแรม (GDDRAM) ของหน้าจอ OLED ภาพบนจอจึงจะเปลี่ยน
