...
ตอบ ปรากฏอยู่ด้านล่างจริง 
รายละเอียดที่สังเกตได้ ทางกายภาพแถบสีเหลือง (16 แถวบน) ควรอยู่ชิดขอบฝั่งขาพินเชื่อมต่อ แต่ผลการแสดงผลจริง ข้อความที่ควรจะอยู่บนแถบสีเหลืองกลับสลับลงไปแสดงผลอยู่บริเวณแถบสีฟ้าด้านล่าง
> 2. **ทิศทางของตัวอักษร** ข้อความ `"HELLO WORLD"` และ `"ID: 65012345"` แสดงผลกลับหัว 180 องศา (Upside-down) หรือไม่?
> คำตอบ แสดงผลกลับหัว 180 องศา (Upside-down) และกลับด้านซ้าย-ขวา (Mirrored) จริง
รายละเอียดที่สังเกตได้ ข้อความ "HELLO WORLD" และรหัสนักศึกษา กลับด้านเหมือนมองผ่านกระจกเงาและกลับหัวจากมุมมองปกติ
...
เกิดจากค่าดีฟอลต์ (Default Reset Value) ของรีจิสเตอร์สแกนพิกเซลฮาร์ดแวร์ภายในชิป SSD1306 ในชุดคำสั่ง Magic Initialization Sequence ยังไม่ได้ระบุคำสั่งควบคุมทิศทางการแมปพิกเซล 2 คำสั่ง ได้แก่

1. Segment Remap (0xA0 / 0xA1) 
ขาดคำสั่งกำหนดทิศทางการแมปแอดเดรสของคอลัมน์ (Column Address to Segment Driver) ทำให้ลำดับพิกเซลแนวนอนถูกกวาดจากขวาไปซ้าย

2. COM Output Scan Direction (0xC0 / 0xC8)
ขาดคำสั่งกำหนดทิศทางการสแกนแถวแนวตั้ง (COM Scan Direction) ทำให้การกวาดแถวแนวตั้งเริ่มจากแถวล่างสุดย้อนขึ้นด้านบน

เมื่อขาดการตั้งค่าทั้งแกนแนวนอนและแนวตั้ง ฮาร์ดแวร์จึงกวาดสแกนพิกเซลจากมุมมองตรงข้ามกับแผงจอจริง ส่งผลให้ภาพที่ปรากฏแสดงผลกลับหัว 180 องศาและกลับด้านซ้าย-ขวา
>
...
#### 1. ตารางบันทึกค่าไบต์และรูปแบบบิตของตัวอักษร 'H' (คอลัมน์ 30 ถึง 34)

| Byte Index | Column Index | Hex Value | Binary Value (Bit 7 -> Bit 0) | บิตที่เป็น 1 |
| :---: | :---: | :---: | :---: | :---: |
...
> หมายเหตุ: Byte[35] หรือ Col 35 มีค่าเป็น `0x00` (`00000000`) ทำหน้าที่เป็นช่องไฟเว้นวรรค 1 พิกเซล

---

#### 2. การถอดรหัสโครงสร้างพิกเซล (Bit-to-Pixel Reconstruction)

การจัดวางบิตในแรมของ SSD1306 ในแต่ละ Page กำหนดให้ **Bit 0 อยู่แถวบนสุด (Row 0 / LSB)** และ **Bit 7 อยู่แถวล่างสุด (Row 7 / MSB)** เมื่อนำค่าบิตมากางเป็นเมทริกซ์พิกเซลขนาด 5 x 8 จะได้โครงสร้างดังนี้:

| แถวพิกเซล (Row / Bit) | Col 30 (`0xF0`) | Col 31 (`0x80`) | Col 32 (`0x80`) | Col 33 (`0x80`) | Col 34 (`0xF0`) | การแสดงผลพิกเซล |
| :--- | :---: | :---: | :---: | :---: | :---: | :--- |
| **Row 0 (Bit 0)** | 0 | 0 | 0 | 0 | 0 | `[ ][ ][ ][ ][ ]` (ช่องว่างด้านบน) |
| **Row 1 (Bit 1)** | 0 | 0 | 0 | 0 | 0 | `[ ][ ][ ][ ][ ]` (ช่องว่างด้านบน) |
| **Row 2 (Bit 2)** | 0 | 0 | 0 | 0 | 0 | `[ ][ ][ ][ ][ ]` (ช่องว่างด้านบน) |
| **Row 3 (Bit 3)** | 0 | 0 | 0 | 0 | 0 | `[ ][ ][ ][ ][ ]` (ช่องว่างด้านบน) |
| **Row 4 (Bit 4)** | **1** | 0 | 0 | 0 | **1** | `[■][ ][ ][ ][■]` (เสาซ้าย-ขวา) |
| **Row 5 (Bit 5)** | **1** | 0 | 0 | 0 | **1** | `[■][ ][ ][ ][■]` (เสาซ้าย-ขวา) |
| **Row 6 (Bit 6)** | **1** | 0 | 0 | 0 | **1** | `[■][ ][ ][ ][■]` (เสาซ้าย-ขวา) |
| **Row 7 (Bit 7)** | **1** | **1** | **1** | **1** | **1** | `[■][■][■][■][■]` (คานกลางตัว H) |

**สรุปผลการพิสูจน์:**
กลุ่มบิต `1` ในคอลัมน์ 30 และ 34 ทำหน้าที่เป็นเส้นแนวตั้งซ้าย-ขวาของตัวอักษร ส่วนบิต `1` ในแถวที่ 7 (Bit 7) ของทุกคอลัมน์ (Col 30–34) ทำหน้าที่เป็นเส้นเชื่อมคานกลาง รูปแบบการเรียงตัวของบิต `1` จึงตรงกับโครงสร้างตัวอักษร `'H'` ในตาราง Font Matrix 5x7 ตามที่แสดงผลบนหน้าจอ OLED จริง
...
### 1. จากการทำ Hex Dump ในกิจกรรมนิติวิทยาศาสตร์ จงอธิบายว่าทำไมตัวอักษร 'H' จึงใช้ข้อมูลจำนวน 5 ไบต์ และแต่ละไบต์ทำหน้าที่ควบคุมพิกเซลในทิศทางใด?
**ตอบ:** 
ตัวอักษรใช้รูปแบบ Font Matrix ขนาดความกว้าง 5 พิกเซล และความสูง 7 พิกเซล โครงสร้างหน่วยความจำของ SSD1306 จัดเก็บข้อมูลแบบ **Page Addressing** โดยใน 1 ไบต์ (8 บิต) จะใช้ควบคุมพิกเซลใน **แนวตั้ง (Vertical)** เรียงจาก Bit 0 (บน) ถึง Bit 7 (ล่าง) ดังนั้น ตัวอักษรที่มีความกว้าง 5 พิกเซล จึงต้องใช้ข้อมูลเรียงต่อกันในแนวนอนจำนวน **5 ไบต์** เพื่อแทน 5 คอลัมน์แนวตั้ง

### 2. หากเราสลับสายไฟระหว่างขา D0 และ D1 จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?
**ตอบ:** 
หน้าจอจะไม่ติดและไม่มีข้อมูลใดแสดงผล (จอดับสนิท) เนื่องจากขา **D0 คือสัญญาณนาฬิกา (SCK/Clock)** ที่ใช้กำหนดจังหวะในการชิฟต์บิตข้อมูล ส่วน **D1 คือสัญญาณข้อมูล (MOSI/Data)** หากต่อสลับสายกัน สัญญาณนาฬิกาและข้อมูลจะผิดพลาดโดยสิ้นเชิง ส่งผลให้คอนโทรลเลอร์ SSD1306 ไม่สามารถอ่านชุดคำสั่ง Magic Initialization Sequence ได้ วงจรทวีแรงดัน (Charge Pump) จึงไม่เปิดทำงาน และไม่สามารถรับส่งข้อมูลพิกเซลได้

### 3. เหตุใดการแก้ไขพิกัด (x, y) บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
**ตอบ:** 
เพราะโปรแกรมใช้ระบบสถาปัตยกรรมแบบ **Frame Buffering (Double Buffering)** การเรียกฟังก์ชัน `oled_draw_pixel()` เป็นเพียงการแก้ไขค่าบิตภายในตัวแปรอะเรย์ `s_oled_buffer` (ขนาด 1,024 ไบต์) บนหน่วยความจำ SRAM ของ ESP32 เท่านั้น ข้อมูลยังไม่ได้ถูกส่งออกไปภายนอกทางฮาร์ดแวร์ จนกว่าจะมีการเรียกคำสั่ง `oled_flush()` ข้อมูลทั้งหมดในแรมจึงจะถูกถ่ายโอนผ่านบัส SPI ไปยังหน่วยความจำ Graphic Display Data RAM (GDDRAM) ภายในชิป SSD1306 เพื่อขับแรงดันไปยังเมทริกซ์พิกเซลบนหน้าจอจริง
