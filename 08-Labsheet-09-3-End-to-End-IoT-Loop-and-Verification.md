...
- เวลาที่ Kestrel ใช้ในการรับคำขอและอัปเดตสถานะ `___0.0381646766666667_______ ms`
- เวลาที่คำสั่งถูกเขียนลงพอร์ต Serial ไปจนถึงจังหวะที่จอ OLED วาดข้อความใหม่สำเร็จ `__2.2898806________ ms`
...
| :---: | :---: | :---: | :---: | :---: | :---: |
| หมุนซ้ายสุด ($0^\circ$) | 0| 0| 0| [x ] ตรง [ ] ไม่ตรง |cloud ready |
| หมุนประมาณ $45^\circ$ | 1090| 24.8|24.9 | [x ] ตรง [ ] ไม่ตรง | cloud ready|
| กึ่งกลาง ($90^\circ$) |2045 |49.9 |49.9 | [ x] ตรง [ ] ไม่ตรง | cloud ready|
| หมุนประมาณ $135^\circ$|3000 |75 | 75| [x ] ตรง [ ] ไม่ตรง | cloud ready|
| หมุนขวาสุด ($180^\circ$)|4095 |100 |100 | [ x] ตรง [ ] ไม่ตรง | cloud ready|
...
### สรุปภาพรวมข้อ 1: การวิเคราะห์จุดคอขวด (Bottleneck Analysis)

หากพบว่าความหน่วงเวลาโดยรวม ($\Delta T$) สูงเกิน 200 ms สาเหตุหลักเกิดจาก **พอร์ต Serial UART (115200 bps)** โดยมีเหตุผลเปรียบเทียบเชิงวิศวกรรมและการคำนวณดังนี้:

| ส่วนประกอบ | อัตราเร็ว / เวลาในการทำงาน | ผลกระทบต่อความหน่วง ($\Delta T$) |
| :--- | :--- | :--- |
| **SPI2 (10 MHz)** | ส่งข้อมูล 1 ไบต์ใช้เวลาเพียง $0.8\ \mu\text{s}$ (แพ็กเก็ต 64 ไบต์ ใช้เวลา $\approx 51.2\ \mu\text{s}$) | อยู่ในระดับไมโครวินาที **ไม่มีผล** ให้เกิดความหน่วงระดับ 200 ms |
| **ADC1 (One-Shot)** | สุ่มตัวอย่างและแปลงสัญญาณ (SAR Conversion) ใช้เวลา $\approx 10\text{--}100\ \mu\text{s}$ ต่อครั้ง | ความเร็วยังอยู่ในระดับไมโครวินาที **ไม่ใช่สาเหตุ** ของปัญหา |
| **UART (115200 bps)** | ส่งข้อมูลแบบ 8-N-1 (10 บิต/ไบต์) ส่งได้ $11,520\text{ ไบต์/วินาที}$ ($\approx 86.8\ \mu\text{s/ไบต์}$) | **เป็นจุดคอขวดหลัก** เนื่องจากความเร็วต่ำที่สุดในระบบ |

#### การคำนวณและการวิเคราะห์เชิงลึกของ UART
* **อัตราการส่งข้อมูลจริง:**
  $$\text{Throughput} = \frac{115200\text{ bps}}{10\text{ bits/byte}} = 11,520\text{ Bytes/s} \implies \text{เวลาต่อไบต์} \approx 86.8\ \mu\text{s}$$
* **พฤติกรรมการบล็อกซีพียู (Blocking Behavior):** 
  หากโปรแกรมมีคำสั่งส่งข้อความแบบปิดกั้นการทำงาน (Blocking I/O เช่น `printf()` หรือ `Serial.println()`) เพื่อส่ง String/JSON ขนาดยาว หรือข้อความ Debug Log บัฟเฟอร์ส่ง (TX FIFO) จะเต็มอย่างรวดเร็ว ทำให้ CPU ต้องหยุดรอ (Block) จนกว่าข้อมูลจะทยอยส่งออกไปทางสายสัญญาณครบ หรือหากคอมพิวเตอร์ฝั่งรับประมวลผลช้า ความหน่วงสะสมนี้จะแตะระดับเกิน **200 ms** ได้ทันที
...
1. **ประโยชน์ของสถาปัตยกรรม Hybrid Edge-Cloud Fallback**  
   ...
   - **เหตุผลที่ต้องมีกลไกนี้:** ป้องกันไม่ให้อินเทอร์เน็ตเป็นจุดตายของระบบ (Single Point of Failure) หากพึ่งพา Cloud เพียงอย่างเดียว เมื่อเน็ตหลุดอุปกรณ์จะทำงานต่อไม่ได้ การมี Edge Fallback ช่วยให้เครื่องจักรทำงานต่อได้เองทันทีแบบ Local Control
   - **ผลเสียหากไม่มีกลไก:** ระบบหยุดชะงักทันทีเมื่อสัญญาณขาดหาย เครื่องจักรขาดการควบคุม อาจก่อให้เกิดอุบัติเหตุชนชิ้นงาน หรือเกิดความเสียหายร้ายแรงต่อระบบความปลอดภัย
...
- **สาเหตุที่ห้ามใช้ ADC2 ร่วมกับ Wi-Fi:** โมดูล Wi-Fi จำเป็นต้องยึด ADC2 ไปใช้ปรับเทียบกำลังสัญญาณวิทยุ (RF Calibration) ทำให้คำสั่งอ่านเซนเซอร์ของผู้ใช้ถูกแย่งคิวและเกิดข้อผิดพลาด
   - **การออกแบบฮาร์ดแวร์เผื่อไว้:** ผู้ผลิตชิปจึงออกแบบบล็อก **ADC1 แยกขาดออกจากวงจรวิทยุโดยสิ้นเชิง** เพื่อเป็นวงจรสำรอง/วงจรเฉพาะสำหรับให้นักพัฒนานำไปใช้อ่านเซนเซอร์ได้อย่างอิสระโดยไม่ชนกับการทำงานของ Wi-Fi
