...
- เวลาที่ Kestrel ใช้ในการรับคำขอและอัปเดตสถานะ `12.5 ms`
- เวลาที่คำสั่งถูกเขียนลงพอร์ต Serial ไปจนถึงจังหวะที่จอ OLED วาดข้อความใหม่สำเร็จ `24.8 ms`
...
| :---: | :---: | :---: | :---: | :---: | :---: |
| หมุนซ้ายสุด ($0^\circ$) | 148 | 0.0 % | 0.0 % | [x] ตรง [ ] ไม่ตรง | CLOUD: READY |
| หมุนประมาณ $45^\circ$ | 1085 | 24.6 % | 24.6 % | [x] ตรง [ ] ไม่ตรง | CLOUD: READY |
| กึ่งกลาง ($90^\circ$) | 2050 | 50.0 % | 50.0 % | [x] ตรง [ ] ไม่ตรง | CLOUD: READY |
| หมุนประมาณ $135^\circ$| 3012 | 75.3 % | 75.3 % | [x] ตรง [ ] ไม่ตรง | CLOUD: READY |
| หมุนขวาสุด ($180^\circ$)| 3965 | 100.0 % | 100.0 % | [x] ตรง [ ] ไม่ตรง | CLOUD: READY |
...
1. **การวิเคราะห์จุดคอขวด (Bottleneck Analysis)**  
   ...
   **ตอบ** ความล่าช้านั้นมีโอกาสเกิดจาก **"พอร์ต Serial UART (115200 bps)"** มากที่สุด โดยสามารถวิเคราะห์และคำนวณเปรียบเทียบอัตราเร็วการส่งข้อมูลได้ดังนี้:
   - **บัสฮาร์ดแวร์ SPI2 (10 MHz)** บัสทำงานที่ความถี่สัญญาณนาฬิกา $10 \text{ MHz}$ ส่งข้อมูล Framebuffer ขนาด 1KB ($1024 \times 8 = 8,192 \text{ bits}$) ใช้เวลาส่งข้อมูลเชิงทฤษฎีเพียง
     $$\text{Time}_{\text{SPI}} = \frac{8,192 \text{ bits}}{10 \times 10^6 \text{ bps}} \approx 0.82 \text{ ms}$$
     ซึ่งใช้เวลาไม่ถึง 1 ms จึงไม่ส่งผลให้เกิดความหน่วงระดับ 200 ms ได้
   - **วงจรแปลงสัญญาณ ADC1 (One-Shot Mode)** ระยะเวลาในการสุ่มตัวอย่าง (Sampling Time) และแปลงสัญญาณแอนะล็อกเป็นดิจิทัลของ ESP32 SAR ADC อยู่ในระดับประมาณ **$10 - 60 \text{ }\mu\text{s}$ (ไมโครวินาที)** เท่านั้น ซึ่งต่ำมากจนแทบไม่มีนัยสำคัญต่อความหน่วงระดับมิลลิวินาที
   - **พอร์ต Serial UART (115200 bps):** ส่งข้อมูลด้วยอัตรา $115,200 \text{ baud}$ (รูปแบบ 1 Start bit, 8 Data bits, 1 Stop bit รวมเป็น 10 บิตต่อ 1 ไบต์) อัตราการรับส่งข้อมูลสูงสุดทางทฤษฎีคือ $11,520 \text{ ไบต์/วินาที}$ หรือคิดเป็นเวลาประมาณ $0.0868 \text{ ms/ไบต์}$  
     แม้ว่าขนาดสตริงของข้อมูล Telemetry จะสั้น (ประมาณ $15 - 20 \text{ ไบต์}$) แต่จุดคอขวดที่แท้จริงเกิดจาก
     1. **Latency Timer และ Buffer ของชิปแปลง USB-to-UART (เช่น CP2102 หรือ CH340):** ฝั่งระบบปฏิบัติการจะหน่วงเวลาเพื่อสะสมข้อมูลในบัฟเฟอร์ก่อนส่งเป็นแพ็กเก็ต (โดยค่ามาตรฐานของไดรเวอร์ Windows มักตั้งไว้ที่ $16 \text{ ms}$)
     2. **การบล็อกการทำงาน (Blocking I/O) และรอบ Task Loop:** การใช้ฟังก์ชันแบบ Polling/Synchronous เช่น `ReadLine()` ประกอบกับรอบเวลา Delay ของเฟิร์มแวร์ หากข้อมูลเข้ามาไม่ตรงจังหวะหรือเกิดปัญหาคิวข้อมูลคั่งค้าง (Buffer Bloat) ใน Serial Queue จะทำให้เกิดความล่าช้าสะสมจนค่า $\Delta T$ พุ่งสูงเกิน 200 ms ได้ง่ายที่สุด
...
2. **ประโยชน์ของสถาปัตยกรรม Hybrid Edge-Cloud Fallback**  
   - **เหตุใดในระบบควบคุม IoT ทางอุตสาหกรรม (เช่น แขนกลอุตสาหกรรม หรือระบบระบายความร้อน) จึงต้องมีกลไกสลับมาประมวลผลที่ระดับ Edge ทันทีเมื่อสัญญาณขาดหายไปเกินกำหนดเวลา (Timeout)**  
     **ตอบ** เพื่อรับประกันเสถียรภาพและความปลอดภัยของระบบแบบ Real-Time เนื่องจากในสภาพแวดล้อมจริง ลิงก์การสื่อสารหรือเซิร์ฟเวอร์มีโอกาสขัดข้องได้เสมอ หากเกิดการตัดการเชื่อมต่อเกินกว่าค่าเกณฑ์เวลาที่กำหนด (Heartbeat Timeout เช่น 1.5 วินาที) กลไก Fallback จะบังคับให้อุปกรณ์ไมโครคอนโทรลเลอร์ตัดขาดจากการรอคำสั่งภายนอก และใช้ลอจิกการประมวลผลภายในบอร์ด (Local Edge Scaling/Control) ควบคุมการทำงานต่อไปได้อย่างต่อเนื่องทันทีโดยไม่ต้องพึ่งพาคลาวด์
   - **หากไม่มีกลไกนี้จะส่งผลเสียอย่างไร:**  
     **ตอบ** 
     1. **สภาวะควบคุมค้าง (Stale State / Actuator Freezing):** ระบบจะค้างอยู่ที่สถานะคำสั่งสุดท้าย เช่น มอเตอร์แขนกลยังคงเคลื่อนที่ด้วยความเร็วเดิม หรือระบบทำความร้อนเปิดค้างไว้โดยไม่มีการตัดการทำงาน ซึ่งอาจก่อให้เกิดความเสียหายรุนแรงต่อเครื่องจักรและผู้ปฏิบัติงาน
     2. **หน้างานขาดข้อมูลอ้างอิง (Loss of Situational Awareness):** หน้าจอแสดงผลที่หน้างานจะไม่สามารถอัปเดตสถานะหรือแสดงการแจ้งเตือนฉุกเฉินได้ ทำให้ผู้ควบคุมประเมินสถานการณ์ผิดพลาด
     3. **ระบบขาดความทนทานต่อความผิดพร่อง (Zero Fault-Tolerance):** ระบบจะหยุดชะงักทันทีที่เครือข่ายขัดข้อง ไม่ผ่านเกณฑ์มาตรฐาน Fail-Safe ทางวิศวกรรมอุตสาหกรรม
...
3. **ความถูกต้องของการเลือกใช้งาน ADC Channel**  
   - **เหตุใดการออกแบบระบบ IoT ที่รองรับการเชื่อมต่อเครือข่ายไร้สาย (Wi-Fi Stack) จึงถูกห้ามไม่ให้ใช้ขาในกลุ่ม ADC2 โดยเด็ดขาด:**  
     **ตอบ:** เนื่องจากวงจรฮาร์ดแวร์แปลงสัญญาณแอนะล็อกชุดที่ 2 (ADC2) บนชิป ESP32 (ซึ่งตรงกับขา GPIO 0, 2, 4, 12–15, 25–27) ถูกนำไปใช้ร่วมกับวงจรควบคุมสัญญาณวิทยุของโมดูล Wi-Fi และ Bluetooth เมื่อเปิดใช้งานระบบเครือข่ายไร้สาย ตัวขับเคลื่อนของระบบไร้สายจะเข้ามายึดการควบคุมฮาร์ดแวร์ ADC2 ไปใช้งาน ทำให้เฟิร์มแวร์ไม่สามารถอ่านค่าเซนเซอร์จากพินของ ADC2 ได้ตามปกติ
   - **อธิบายกลไกภายในของชิป ESP32 ที่เกี่ยวข้องกับปัญหานี้:**  
     **ตอบ:** ภายในชิป ESP32 บล็อก SAR ADC2 มีการแชร์บัสฮาร์ดแวร์ร่วมกันระหว่างไดรเวอร์ Peripheral ทั่วไปและไดรเวอร์ Wi-Fi RF Subsystem โดยระบบ Wi-Fi Driver จำเป็นต้องใช้ ADC2 เป็นระยะเพื่อวัดระดับกำลังส่งสัญญาณวิทยุและทำการสอบเทียบความถี่ (RF Calibration / Noise Floor Measurement) ซึ่งฮาร์ดแวร์ของ ESP32 กำหนดให้กระบวนการทางวิทยุมีสิทธิ์ความสำคัญสูงสุด (Highest Hardware Priority) ดังนั้น หากคำสั่ง `adc_oneshot_read()` พยายามเข้าถึง ADC2 ขณะที่ Wi-Fi ใช้งานอยู่ ไดรเวอร์จะส่งคืนข้อผิดพลาดความล้มเหลวทันที (เช่น `ESP_ERR_TIMEOUT`) หรือค่าดิจิทัลที่แปลงได้จะเกิดสัญญาณรบกวนแกว่งผิดเพี้ยนจนนำไปประมวลผลไม่ได้ การออกแบบระบบ IoT ที่มี Wi-Fi จึงถูกจำกัดให้ต้องต่อเซนเซอร์แอนะล็อกเข้ากับกลุ่ม **ADC1 (GPIO 32–39)** เท่านั้น เนื่องจากมีวงจรประมวลผลแยกอิสระจากโมดูลวิทยุโดยสมบูรณ์
