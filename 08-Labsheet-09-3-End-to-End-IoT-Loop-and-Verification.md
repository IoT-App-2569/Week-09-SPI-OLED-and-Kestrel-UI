...
void render_multizone_ui(int raw_val, int percent, bool is_cloud, const char *msg) {
    ...
    char header_str[32];
    snprintf(header_str, sizeof(header_str), "ESP32 | 67030296"); // <-- แก้ไขเป็นรหัสนักศึกษาจริง
...
- เวลาที่ Kestrel ใช้ในการรับคำขอและอัปเดตสถานะ 42  ms`
- เวลาที่คำสั่งถูกเขียนลงพอร์ต Serial ไปจนถึงจังหวะที่จอ OLED วาดข้อความใหม่สำเร็จ 30 ms`
...
| :----------------------: | :-----------------------------: | :---------------------------: | :------------------: | :---------------------------------: | :------------------: |
| หมุนซ้ายสุด ($0^\circ$)  |                0                |               0               |          0           |         [x] ตรง [ ] ไม่ตรง          |  CLOUD system ready  |
|  หมุนประมาณ $45^\circ$   |              1100               |              25               |          25          |         [x] ตรง [ ] ไม่ตรง          |  CLOUD system ready  |
|  กึ่งกลาง ($90^\circ$)   |              2042               |              50               |          50          |         [x] ตรง [ ] ไม่ตรง          |  CLOUD system ready  |
|  หมุนประมาณ $135^\circ$  |              3000               |              75               |          75          |         [x] ตรง [ ] ไม่ตรง          |  CLOUD system ready  |
| หมุนขวาสุด ($180^\circ$) |              4095               |              100              |         100          |         [x] ตรง [ ] ไม่ตรง          |  CLOUD system ready  |
...
ตอบ ถ้า ΔT เกิน 200 ms ส่วนที่น่าจะเป็นสาเหตุมากที่สุดคือ **พอร์ต Serial UART (115200 bps)** เมื่อเทียบกับอีกสองส่วนซึ่งเร็วกว่ามาก จากการคำนวณอัตราเร็วการส่งข้อมูล UART 115200 bps ส่งได้ประมาณ 11,520 ไบต์ต่อวินาที (1 ไบต์ใช้ 10 บิตรวม start และ stop bit) หรือประมาณ 87 µs ต่อไบต์ แพ็กเก็ตขาขึ้น `ADC:xxxx,xxxxxx\n` ยาวราว 16 ไบต์ ใช้เวลา 1.4 ms และแพ็กเก็ตขาลง `SET:xx:MESSAGE\n` ยาวราว 18 ไบต์ ใช้เวลา 1.6 ms รวมทั้งสองทางประมาณ 3 ms ขณะที่ SPI2 ที่ 10 MHz ส่ง framebuffer 1 KB (8,192 บิต) ใช้เวลาไม่ถึง 1 ms (ประมาณ 0.8 ms บวก overhead รวมราว 1-2 ms) และ ADC1 One-Shot ใช้เวลาระดับสิบไมโครวินาทีต่อการอ่านหนึ่งครั้ง จึงเล็กน้อยมาก จากตัวเลขนี้ทั้ง SPI และ ADC ไม่มีทางทำให้เกิน 200 ms ได้ ส่วน UART แม้จะช้าที่สุดในสามส่วน แต่เวลาส่งจริงก็เพียง 3 ms เท่านั้น ดังนั้นถ้า ΔT เกิน 200 ms จริง สาเหตุคือ UART ถูกใช้งานเกินกำลังหรือเกิดการคิวสะสม เช่น ส่งแพ็กเก็ตถี่กว่าที่ลิงก์รับได้ (Kestrel ตอบทุกแพ็กเก็ตที่รับ ทำให้บัฟเฟอร์ขาเข้าของ ESP32 ขนาด 256 ไบต์ค้างและข้อมูลที่เห็นเป็นข้อมูลเก่า) หรือมีการอ่านบรรทัดไม่ทัน ผลจากการทดลองจริงยืนยันเรื่องนี้ เพราะเวลาที่วัดได้อยู่ที่ประมาณ 42 ms (ซึ่งรวมเวลาสตาร์ต curl แล้ว) และ ΔT โดยประมาณราว 30-60 ms โดยส่วนที่ใหญ่ที่สุดคือรอบลูป 50 ms ของเฟิร์มแวร์ ไม่ใช่ตัวฮาร์ดแวร์บัสเอง

3. **ประโยชน์ของสถาปัตยกรรม Hybrid Edge-Cloud Fallback**  
...
ตอบ ในระบบควบคุมอุตสาหกรรม เช่น แขนกลหรือระบบระบายความร้อน อุปกรณ์ปลายทางต้องตัดสินใจและควบคุมตัวเองได้ต่อเนื่องแม้ลิงก์ไปยังเซิร์ฟเวอร์จะขาด เพราะเครือข่ายและเซิร์ฟเวอร์เป็นจุดที่มีโอกาสล้มเหลวสูง (สายหลุด โปรแกรมค้าง เซิร์ฟเวอร์รีสตาร์ต) การมีกลไก timeout (ในแล็บนี้คือ 1.5 วินาที) แล้วสลับกลับมาคำนวณที่ Edge ทันที ทำให้ระบบยังอ่านเซนเซอร์และคำนวณค่าควบคุมได้เองในเครื่อง ระบบจึงยังทำงานได้ในระดับที่ปลอดภัยแม้ไม่มีคลาวด์ (fail-safe / graceful degradation) และเมื่อการเชื่อมต่อกลับมา ระบบก็ยกระดับเข้าสู่ Cloud Mode อัตโนมัติโดยไม่ต้องรีเซ็ตฮาร์ดแวร์ ถ้าไม่มีกลไกนี้ เมื่อสัญญาณขาด อุปกรณ์จะค้างอยู่กับค่าคำสั่งล่าสุดที่เคยได้รับ หรือหยุดตอบสนองไปเลย เช่น แขนกลอาจเคลื่อนที่ต่อหรือหยุดผิดตำแหน่งจนเกิดอันตรายต่อคนและชิ้นงาน ระบบระบายความร้อนอาจไม่ปรับตามอุณหภูมิจริงจนเครื่องจักรร้อนเกินและเสียหาย นอกจากนี้ผู้ใช้จะได้ข้อมูลที่ไม่ตรงกับความจริงโดยไม่รู้ตัว ทำให้เกิดความเสียหายต่อทั้งความปลอดภัยและความต่อเนื่องของการผลิต
...
ตอบ ESP32 มีวงจร ADC สองชุด คือ ADC1 (GPIO 32-39) และ ADC2 (GPIO 0, 2, 4, 12-15, 25-27) แต่ ADC2 ถูกใช้ร่วมกับโมดูล Wi-Fi ภายในชิป เมื่อ Wi-Fi driver ทำงาน ตัว driver จะเข้ายึดและควบคุม ADC2 (ผ่านการจัดการ RF/ตัวควบคุมพลังงานของโมดูลวิทยุ) เพื่อใช้งานภายในกระบวนการวิทยุ ทำให้แอปพลิเคชันไม่สามารถเข้าใช้ ADC2 พร้อมกันได้ การพยายามอ่านค่าจึงเกิดการแย่งใช้ทรัพยากรฮาร์ดแวร์เดียวกัน ผลคือได้ค่าที่เพี้ยนไม่น่าเชื่อถือ หรือฟังก์ชันอ่านค่าคืน error (เช่น ESP_ERR_TIMEOUT) และในบางกรณีทำให้ระบบทำงานผิดพลาดหรือค้าง ส่วน ADC1 ไม่ถูก Wi-Fi ครอบครอง จึงอ่านค่าได้ตลอดเวลาที่ Wi-Fi หรือ Bluetooth เปิดใช้งาน ดังนั้นการออกแบบระบบ IoT ที่ต้องใช้เครือข่ายไร้สายจึงต้องต่อเซนเซอร์แอนะล็อกเข้ากับขา ADC1 เท่านั้น ซึ่งในแล็บนี้ใช้ GPIO 34 (ADC1 Channel 6) ตามที่ออกแบบไว้
...
<img width="3060" height="4080" alt="20260921_103250" src="https://github.com/user-attachments/assets/b111f6c4-2a06-4311-a7ad-7a07acee044f" />
<img width="3060" height="4080" alt="20260921_103535" src="https://github.com/user-attachments/assets/acec7534-f148-42d9-9451-57b00ebba44a" />
<img width="3060" height="4080" alt="20260921_103244" src="https://github.com/user-attachments/assets/8f99b363-f66b-4b0f-8850-b4289a8f6a77" />
