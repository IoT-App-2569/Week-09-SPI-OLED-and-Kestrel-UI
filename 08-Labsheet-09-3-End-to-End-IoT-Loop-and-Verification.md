...
## ปิด Kestrel
<img width="3024" height="4032" alt="IMG_4489" src="https://github.com/user-attachments/assets/9cc54436-fd0a-4e6c-9681-c8b889157874" />

## เปิด Kestrel อีกครั้ง
<img width="3024" height="4032" alt="IMG_4491" src="https://github.com/user-attachments/assets/d79451d0-cc13-4b0b-995c-e934ba14a983" />
...
https://github.com/user-attachments/assets/74673355-d3cd-49dc-b335-a699f5d23644
...
- เวลาที่ Kestrel ใช้ในการรับคำขอและอัปเดตสถานะ ` 10 ms`
- เวลาที่คำสั่งถูกเขียนลงพอร์ต Serial ไปจนถึงจังหวะที่จอ OLED วาดข้อความใหม่สำเร็จ ` 10 ms`
<img width="792" height="109" alt="image" src="https://github.com/user-attachments/assets/e6b45c8d-1859-44ef-af46-4643622ed8ec" />
...
| :---: | :---: | :---: | :---: | :---: | :---: |
| หมุนซ้ายสุด ($0^\circ$) |0 |0 |0 | [/] ตรง [ ] ไม่ตรง | CLOUD COMPUTING|
| หมุนประมาณ $45^\circ$ | 1156| 24.3|24.3 | [/] ตรง [ ] ไม่ตรง |CLOUD COMPUTING |
| กึ่งกลาง ($90^\circ$) | 2186| 51.3| 51.3| [/] ตรง [ ] ไม่ตรง | CLOUD COMPUTING|
| หมุนประมาณ $135^\circ$| 3069| 76.7|76.7 | [/] ตรง [ ] ไม่ตรง | CLOUD COMPUTING|
| หมุนขวาสุด ($180^\circ$)|4095 |100|100 | [/] ตรง [ ] ไม่ตรง |CLOUD COMPUTING |
...
```
ถ้า ΔT รวมสูงเกิน 200 ms ตัวที่น่าจะเป็นสาเหตุมากที่สุดคือ พอร์ต Serial UART (115200 bps)
เหตุผลจากการคำนวณอัตราเร็วการส่งข้อมูล: SPI2 ที่ความถี่ 10 MHz ใช้เวลาส่ง Framebuffer เต็มจอ 1024 ไบต์ = 8192 บิต
        8192 ÷ 10,000,000 ≈ 0.82 ms ต่อการ flush หนึ่งครั้ง 
        ADC1 ในโหมด One-Shot ใช้เวลาสุ่มตัวอย่างระดับไมโครวินาทีเท่านั้น แทบไม่มีผลต่อ ΔT เลย

ในขณะที่ UART ที่ 115200 bps มีบิตคาบประมาณ 8.68 µs ต่อบิต และแบบ 8N1 ใช้ ~10 บิตต่อไบต์ จึงใช้เวลาประมาณ 86.8 µs ต่อไบต์
เมื่อคิดเป็นข้อความ ADC:xxxx,xxxxxxxx\n ที่มีความยาวประมาณ 18-20 ไบต์ จะใช้เวลาส่งประมาณ 1.5-1.7 ms ต่อทิศทาง และถ้านับรวมทั้งขาไปและขากลับ จะรวมได้ประมาณ 3-4 ms ต่อรอบ

เมื่อเทียบอัตราเร็วดิบกันแล้ว UART มีแบนด์วิดท์เพียง 115.2 kbps ในขณะที่ SPI2 มี 10 Mbps ต่างกันเกือบ 87 เท่า ทำให้ UART เป็นท่อที่แคบที่สุดในระบบทั้งหมด นอกจากนี้ UART0 ยังถูกใช้ร่วมกับการพิมพ์ log ผ่าน ESP_LOG/printf ด้วย จึงมีโอกาสเกิดการแย่งใช้บัฟเฟอร์ และการบล็อกจากฟังก์ชัน SerialPort.ReadLine() ฝั่ง Kestrel  จึงเป็นจุดที่มีโอกาสสะสมความหน่วงจนดันค่า ΔT รวมให้พุ่งสูงผิดปกติมากที่สุด
```
...
```
ระบบควบคุมอุตสาหกรรมต้องมีกลไกสลับมาทำงานที่ Edge ทันทีเมื่อ Timeout เพราะ การเชื่อมต่อเครือข่าย คลาวด์ไม่สามารถรับประกันความเสถียร 100% ได้ wifiอาจหลุด เน็ตดับ แต่กระบวนการทางกายภาพ ยังคงเปลี่ยนแปลงต่อเนื่องแบบ Real-time  การให้ Edge ประมวลผลด้วยข้อมูลเซนเซอร์ที่อ่านได้ ณ ขณะนั้นเอง ทำให้ระบบตอบสนองได้และปลอดภัย แม้ไม่มีคลาวด์ ถือเป็นการออกแบบแบบ Fail-Safe

หากไม่มีกลไกนี้
       - ระบบจะค้างอยู่กับคำสั่งหรือค่าล่าสุดที่ได้รับจากคลาวด์ ทั้งที่จริงเปลี่ยนไปแล้ว
       - แขนกลอาจเคลื่อนที่ต่อตามคำสั่งเก่าจนชน หรือระบบระบายความร้อนไม่ปรับพัดลม วาล์วตามอุณหภูมิจริง จนอุปกรณ์ overheat เสียหายหรือเกิดอันตราย
ระบบทั้งหมดพึ่งพาเครือข่ายเป็น Single Point of Failure ทำให้ไม่ผ่านมาตรฐาน Real-time/Safety ของงานอุตสาหกรรม
```
...
```
ตัวควบคุม Wi-Fi ของ ESP32 ต้องใช้วงจร ADC2 ผ่านทาง RTC controller/ADC arbiter เพื่อสุ่มตรวจสอบและปรับเทียบกำลังส่งสัญญาณ ระหว่างที่ Wi-Fi ทำงานโดยเฉพาะช่วงส่งแพ็กเก็ต เมื่อ Wi-Fi driver ทำงานอยู่ มันจะ จองใช้งาน ADC2 โดยอัตโนมัติ ทำให้ถ้าโปรแกรมแอปพลิเคชันพยายามอ่านค่าจาก ADC2 พร้อมกัน จะเกิดการแย่งกัน ส่งผลให้
       - อ่านค่าได้ผิดเพี้ยน ไม่นิ่ง
       - เกิด Timeout หรือค่าที่อ่านได้ค้าง สุ่ม
       - ในบางกรณีทำให้ระบบค้าง
ส่วน ADC1 เป็นวงจรอิสระที่ไม่มีการเชื่อมโยงกับวงจรภายในของ Wi-Fi module เลย จึงเป็นตัวเลือกเดียวที่ Espressif แนะนำให้ใช้อ่านค่าแอนะล็อกในงาน IoT ที่ต้องเปิด Wi-Fi พร้อมกัน
```
