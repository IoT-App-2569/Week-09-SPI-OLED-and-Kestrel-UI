...
void render_multizone_ui(int raw_val, int percent, bool is_cloud, const char *msg) {
    ...
    char header_str[32];
    snprintf(header_str, sizeof(header_str), "ESP32 | 67030142  "); // <-- แก้ไขเป็นรหัสนักศึกษาจริง
...
**ผลลัพธ์จากการรันคำสั่งจับเวลา (Measure-Command Output):**
```text
Days              : 0
Hours             : 0
Minutes           : 0
...
Milliseconds      : 51
Ticks             : 510423
TotalDays         : 5.90767361111111E-07
TotalHours        : 1.41784166666667E-05
TotalMinutes      : 0.000850705
TotalSeconds      : 0.0510423
TotalMilliseconds : 51.0423
```

*บันทึกค่า `TotalMilliseconds` ที่ได้และการแยกแยะเวลาหน่วงย่อย:*
- เวลาที่ Kestrel ใช้ในการรับคำขอ HTTP, Deserialization, และอัปเดตสถานะภายใน (`T_1 - T_0`): `12.4 ms`
- เวลาที่คำสั่งถูกเขียนลงพอร์ต Serial UART ไปจนถึงจังหวะที่ ESP32 รับข้อมูลและสั่ง SPI Flush วาดจอ OLED สำเร็จ (`T_2 - T_1`): `38.6 ms`
- **เวลารวมของกระบวนการ Round-Trip End-to-End Latency ($\Delta T$):** `51.04 ms`
- **การประเมินผล:** เวลาหน่วงโดยรวมต่ำกว่า 100 ms ตามเกณฑ์มาตรฐาน Real-Time ทางอุตสาหกรรม (PASS)
...
| หมุนประมาณ $45^\circ$ | 1115 | 25 % | 25 % | [X] ตรง [ ] ไม่ตรง | CLOUD: READY |
| กึ่งกลาง ($90^\circ$) | 2050 | 50 % | 50 % | [X] ตรง [ ] ไม่ตรง | CLOUD: READY |
...
1. **การวิเคราะห์จุดคอขวด (Bottleneck Analysis)**  
   ...
   **คำตอบ:**  
   จุดคอขวดคือ **พอร์ต Serial UART (115200 bps)**
   - **SPI2 (10 MHz):** ส่ง Framebuffer 1 KB ใช้เวลาเพียง $\approx 0.82\text{ ms}$
   - **ADC1:** ใช้เวลาแปลงสัญญาณเร็วมาก $< 0.1\text{ ms}$
   - **Serial UART:** มีความเร็วส่งข้อมูลช้าที่สุด และหากข้อมูลคั่งค้างใน FIFO Buffer หรือติด Delay ในโปรแกรม จะทำให้ Latency สะสมเกิน 200 ms ได้ง่ายที่สุด
...
2. **ประโยชน์ของสถาปัตยกรรม Hybrid Edge-Cloud Fallback**  
   ...
   **คำตอบ:**  
   - **เหตุผล:** เพื่อความปลอดภัย (Fail-Safe) เมื่อเน็ตหรือเซิร์ฟเวอร์หลุด ESP32 ต้องสลับมาควบคุมระบบด้วยตนเองได้ทันที
   - **ผลเสียหากไม่มี:** ระบบจะค้างรอคำสั่งจากเซิร์ฟเวอร์ (Blind Operation) ทำให้ควบคุมเครื่องจักรไม่ได้และอาจเกิดอุบัติเหตุหรือความเสียหายร้ายแรง
...
3. **ความถูกต้องของการเลือกใช้งาน ADC Channel**  
   ...
   **คำตอบ:**  
   - **เหตุผล:** ฮาร์ดแวร์ **ADC2** ใช้แชร์ร่วมกับวงจรวิทยุ **Wi-Fi / Bluetooth** ภายในชิป ESP32
   - เมื่อเปิดใช้ Wi-Fi วงจรจะถูกจองใช้งานเพื่อวัดสัญญาณ RF ทำให้ไม่สามารถอ่านค่า ADC2 ได้ หรือค่าที่อ่านได้จะเพี้ยน จึงต้องใช้ขาของ **ADC1 (GPIO 32-39)** เท่านั้น
