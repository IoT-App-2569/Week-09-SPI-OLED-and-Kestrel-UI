...
1. **เหตุใดการคำนวณสเกลเซนเซอร์จึงควรทำที่ฝั่ง Kestrel Server แทนที่จะคำนวณบนไมโครคอนโทรลเลอร์ ESP32 ตั้งแต่แรก?**
   * **คำตอบ:** 
     1. **Dynamic Re-calibration Without Reflashing:** สามารถเปลี่ยนหรือปรับเทียบพารามิเตอร์ของเซนเซอร์ (Zero & Span Point) รวมถึงเปลี่ยนหน่วยการแสดงผล (เช่น % หรือ RPM) ได้แบบ Real-time ผ่าน API โดยไม่ต้องหยุดการทำงานของอุปกรณ์หรือ Re-flash เฟิร์มแวร์ใหม่
     2. **Raw Data Preservation & Decoupling:** การส่งค่า Raw ADC (0–4095) ออกมาตรง ๆ ช่วยรักษาความสมบูรณ์ของข้อมูลดิบดั้งเดิมไว้สำหรับวิเคราะห์เชิงนิติวิทยาศาสตร์ (Audit & Traceability) และลดภาระการคำนวณเลขทศนิยม (Floating Point) บนฮาร์ดแวร์ ESP32
...
2. **จากการทำ HTTP Forensics หากไม่มีการตรวจสอบเงื่อนไข `RawMax <= RawMin` ในโค้ด จะเกิด Exception ชนิดใดขึ้นในภาษา C# และส่งผลต่อการทำงานของเซิร์ฟเวอร์อย่างไร?**
   * **คำตอบ:** 
     * หาก `RawMax == RawMin` ตัวหาร `(_settings.RawMax - _settings.RawMin)` จะเป็นศูนย์ (Divide-by-Zero) ในการคำนวณชนิด `double` จะได้ผลลัพธ์เป็น `NaN` (Not a Number) หรือ `Infinity` ซึ่งเมื่อ Kestrel พยายามแปลงออบเจกต์เป็น JSON ส่งกลับ จะทำให้เกิด **`JsonException`** (The value is not supported by JSON serializer) และส่งสถานะ **`500 Internal Server Error`** ตอบกลับไปยังไคลเอนต์ ทำให้ระบบหยุดการให้บริการ Telemetry ทันที
     * การดักด้วย `ArgumentException` และตอบกลับด้วย **`400 Bad Request`** จึงช่วยป้องกันไม่ให้เซิร์ฟเวอร์แครช (No Server Crash)
...
3. **อธิบายสาเหตุทางเทคนิคว่าทำไมคำขอ HTTP POST ที่ไม่มี Header `Content-Type: application/json` จึงถูกปฏิเสธด้วยรหัสสถานะ `415 Unsupported Media Type`?**
   * **คำตอบ:** 
     * สถาปัตยกรรมของ ASP.NET Core Kestrel อาศัยกลไก **Model Binding & Input Formatter** ในการแปลง Payload จาก Request Body เข้าสู่ C# Class Model
     * เซิร์ฟเวอร์จำเป็นต้องอ่าน Header `Content-Type` เพื่อเลือก Formatter ให้ตรงกับชนิดข้อมูล หากไคลเอนต์ไม่ได้ระบุ `Content-Type: application/json` ระบบจะไม่สามารถเลือก JSON Formatter สำหรับถอดรหัส (Deserialize) ข้อมูลได้ จึงปฏิเสธคำขอตามมาตรฐาน HTTP ด้วยรหัส **`415 Unsupported Media Type`**
