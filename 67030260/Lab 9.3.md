...
ADC:2930,6389
```
![alt text](Images/S__72851612.jpg)
![alt text](Images/S__72851614.jpg)
...
- เวลาที่ Kestrel ใช้ในการรับคำขอและอัปเดตสถานะ `2251.52 ms`
- เวลาที่คำสั่งถูกเขียนลงพอร์ต Serial ไปจนถึงจังหวะที่จอ OLED วาดข้อความใหม่สำเร็จ `27.6 ms`
...
| :---: | :---: | :---: | :---: | :---: | :---: |
| หมุนซ้ายสุด ($0^\circ$) | 0 | 0.0% | 0.0% | [✅] ตรง [ ] ไม่ตรง | CLOUD: READY |
...
1. **การวิเคราะห์จุดคอขวด (Bottleneck Analysis)**  
   ...
   **ตอบ:** **พอร์ต Serial UART (115200 bps)**
   - **เหตุผล:**
     - **ADC1:** แปลงสัญญาณเร็วมากในระดับไมโครวินาที (~0.02 ms)
     - **SPI2 (10 MHz):** ส่ง Framebuffer 1KB ขึ้นจอใช้เวลาเพียง ~0.82 ms
     - **Serial UART:** เป็นส่วนที่ช้าที่สุด (115.2 kbps ช้ากว่า SPI เกือบ 100 เท่า) และมีความหน่วงจาก OS Serial Buffer / การรออ่านข้อมูลแบบ Blocking I/O (`ReadTimeout`) หากเกิดคิวคั่งค้าง จะทำให้ Latency สะสมพุ่งเกิน 200 ms ได้ง่ายที่สุด
...
2. **ประโยชน์ของสถาปัตยกรรม Hybrid Edge-Cloud Fallback**  
   ...
   **ตอบ:**
   - **เหตุผลที่ต้องมี:** เพื่อความต่อเนื่องในการทำงาน (High Availability) และความปลอดภัย (Fail-Safe) หากเครือข่ายหรือคลาวด์หลุด ESP32 จะสลับมาคำนวณสเกลเองที่หน้างานแบบ Real-time ได้ทันทีโดยไม่หยุดชะงัก
   - **ผลเสียหากไม่มี:** ระบบจะสูญเสียการควบคุมและค้างอยู่ที่สถานะคำสั่งสุดท้าย (Loss of Control) เช่น แขนกลหมุนค้างจนชนโครงสร้าง หรือระบบระบายความร้อนหยุดทำงานจนเครื่องจักรพังเสียหายร้ายแรง
...
3. **ความถูกต้องของการเลือกใช้งาน ADC Channel**  
   ...
   **ตอบ:**
   - **เหตุผลและกลไกภายใน:** วงจร **SAR ADC2 แชร์ฮาร์ดแวร์ร่วมกับโมดูลวิทยุ Wi-Fi/Bluetooth** (ใช้ตรวจวัดสัญญาณ RF และ Noise Floor) เมื่อเปิดใช้งาน Wi-Fi ตัว Wi-Fi Driver จะยึดครองวงจร ADC2 แบบสิทธิ์ขาด ทำให้ระบบอ่านค่าแอนะล็อกภายนอกไม่ได้ (เกิด Timeout / ค่าเพี้ยน)
   - **แนวทางที่ถูกต้อง:** ต้องเลือกใช้ขาในกลุ่ม **ADC1** (เช่น GPIO 34 ในแล็บนี้) เท่านั้น เพราะมีวงจรแปลงสัญญาณแยกอิสระ ไม่ถูก Wi-Fi รบกวน
