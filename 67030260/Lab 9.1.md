...
```
II (27) boot: ESP-IDF v6.0.2 2nd stage bootloader
I (27) boot: compile time Sep 14 2026 10:07:44
I (28) boot: Multicore bootloader
I (29) boot: chip revision: v3.1
I (32) boot.esp32: SPI Speed      : 40MHz
...
```c
    ...
    oled_draw_string(24, 32, "ID: 67030260", true);  // โซนสีฟ้า
    oled_flush();
```
...
I (191) app_init: App version:      1
I (195) app_init: Compile time:     Sep 14 2026 10:07:16
I (200) app_init: ELF file SHA256:  45b94acf5...
I (204) app_init: ESP-IDF:          v6.0.2
I (208) efuse_init: Min chip rev:     v0.0
I (212) efuse_init: Max chip rev:     v3.99
I (216) efuse_init: Chip rev:         v3.1
...
* **คำตอบ:** 
ตัวอักษร 'H' ใช้ข้อมูล 5 ไบต์เนื่องจากฟอนต์บิตแมปแบ่งออกเป็น 5 คอลัมน์แนวนอน โดยแต่ละบิตใน 1 ไบต์จะทำหน้าที่ควบคุมพิกเซลในแนวตั้ง ($y$-axis) และลำดับของทั้ง 5 ไบต์จะเรียงต่อกันเพื่อควบคุมตำแหน่งคอลัมน์ในแนวนอน ($x$-axis) จากซ้ายไปขวา
2. หากเราสลับสายไฟระหว่างขา **D0** และ **D1** จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?
* **คำตอบ:** 
ขา D0 คือสัญญาณนาฬิกา (SCK) และ D1 คือสัญญาณข้อมูล (MOSI) เมื่อสลับสายกัน ข้อมูลและจังหวะนาฬิกาจะเข้าผิดช่องทาง ทำให้สัญญาณ SPI ไม่สามารถซิงโครไนซ์เพื่อถอดรหัสข้อมูลได้ ส่งผลให้หน้าจอไม่ติด หรือแสดงผลเป็นภาพขยะ
3. เหตุใดการแก้ไขพิกัด $(x, y)$ บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
* **คำตอบ:** 
การแก้ไข s_oled_buffer เป็นเพียงการอัปเดตข้อมูลภาพใน RAM ของไมโครคอนโทรลเลอร์ (MCU) เท่านั้น ภาพบนหน้าจอจริงจะเปลี่ยนแปลงก็ต่อเมื่อมีการเรียกคำสั่ง oled_flush() เพื่อคัดลอกและส่งชุดข้อมูลทั้งหมดผ่านบัส SPI เข้าสู่หน่วยความจำ GDDRAM ของจอ OLED
