...
1. เหตุใดจึงควรคำนวณสเกลเซนเซอร์ที่ Kestrel Server?

เพราะแก้ไขค่า Calibration ได้ง่าย ไม่ต้องแก้และลงโปรแกรมใหม่ใน ESP32 และสามารถจัดการค่าจากหลายอุปกรณ์ได้จาก Server


---

2. ถ้าไม่มีการตรวจสอบ `RawMax <= RawMin` จะเกิดอะไรขึ้น?

ถ้า `RawMax` เท่ากับ `RawMin` จะทำให้เกิดการหารด้วยศูนย์ และค่าที่คำนวณได้อาจผิดพลาด ส่วนถ้า `RawMax` น้อยกว่า `RawMin` จะทำให้การ Calibration ผิดช่วง ดังนั้นจึงต้องตรวจสอบก่อนคำนวณ


---

3. ทำไม POST ที่ไม่มี `Content-Type: application/json` จึงได้ `415 Unsupported Media Type`?

เพราะ Server ไม่รู้ว่า Request Body ที่ส่งมาเป็นข้อมูลรูปแบบ JSON จึงไม่สามารถนำข้อมูลไปประมวลผลตามที่ API ต้องการ และตอบกลับ `415 Unsupported Media Type`
