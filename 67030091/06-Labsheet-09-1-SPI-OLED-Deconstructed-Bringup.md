...
* **ผลที่สังเกตได้:** แถบสีเหลือง (ขนาด 16 พิกเซลดั้งเดิม) สลับลงไปอยู่ **ขอบล่างของหน้าจอ** ส่วนแถบสีฟ้า (48 พิกเซล) ขึ้นมาอยู่ทางด้านบนแทน
* **ผลของข้อความ:** ข้อความ `"HELLO WORLD"` ที่กำหนดพิกัด $y=4$ ซึ่งควรอยู่โซนบน กลับไปสว่างอยู่ที่แถบสีฟ้าด้านบนแทนที่จะเป็นแถบสีเหลือง
...
-  2. ทิศทางของตัวอักษรและการแสดงผล
* **ผลที่สังเกตได้:** ตัวอักษร `"HELLO WORLD"` และรหัสนักศึกษา แสดงผล **กลับหัว 180 องศา (Upside-down)** และเรียงลำดับจากขวาไปซ้าย (Mirrored)
* **สาเหตุเชิงนิติวิทยาศาสตร์:** ใน `Magic Sequence` เริ่มต้น เรายังไม่ได้ส่งคำสั่งตั้งค่ารีจิสเตอร์สลับทิศทางการสแกนของฮาร์ดแวร์ ได้แก่:
  1. คำสั่ง **Segment Remap (`0xA0` / `0xA1`)** ซึ่งคุมทิศทางการสแกนแนวนอน (ซ้าย $\leftrightarrow$ ขวา)
  2. คำสั่ง **COM Output Scan Direction (`0xC0` / `0xC8`)** ซึ่งคุมทิศทางการสแกนแนวตั้ง (บน $\leftrightarrow$ ล่าง)  
  เมื่อคอนโทรลเลอร์ทำงานด้วยค่าเริ่มต้นของโรงงาน ทิศทางการแมปฮาร์ดแวร์จึงไม่ตรงกับแนววางพาเนลจอ ทำให้ภาพหมุนกลับหัว 180 องศา
...
**คำตอบ:**
* **ทำไมถึงใช้ 5 ไบต์:** เพราะฟอนต์ที่ใช้เป็นแบบ **Font Matrix 5x7 พิกเซล** หมายถึงตัวอักษรหนึ่งตัวจะมีความกว้าง 5 พิกเซล และสูง 7 พิกเซล โครงสร้างของจอ SSD1306 จัดการแรมแบบแบ่งเป็น Page ละ 8 บิตในแนวตั้ง การนำข้อมูลฟอนต์มาวาดจึงเก็บข้อมูลทีละ 1 คอลัมน์ลงใน 1 ไบต์ ทำให้ความกว้าง 5 คอลัมน์ต้องใช้ข้อมูลทั้งหมด **5 ไบต์** พอดี (บิตที่ 0–6 ใช้แสดงผลจุดพิกเซล ส่วนบิตที่ 7 ปล่อยว่าง)
* **ทิศทางการควบคุมพิกเซล:** แต่ละไบต์จะทำหน้าที่ควบคุมพิกเซลใน **แนวตั้ง (Vertical Column)** จากบนลงล่าง โดยบิตที่มีค่าน้อย (LSB: Bit 0) จะคุมพิกเซลแถวบนสุด และบิตที่มีค่ามากขึ้น (Bit 1, 2, ... จนถึง Bit 6) จะไล่ลงมาคุมพิกเซลแถวล่างตามลำดับ
...
**คำตอบ:**
* **ผลกับสัญญาณ SPI:** 
  * ขา **D0** คือสัญญาณ **SCK (SPI Clock)** ทำหน้าที่สร้างจังหวะสัญญาณนาฬิกาเพื่อบอกให้อีกฝั่งอ่านข้อมูล
  * ขา **D1** คือสัญญาณ **MOSI (Data)** ทำหน้าที่ส่งบิตข้อมูลคำสั่งและพิกเซล  
  ถ้าสลับสายกัน สัญญาณนาฬิกาจะวิ่งเข้าช่องรับข้อมูล และข้อมูลบิตจะวิ่งเข้าช่องสัญญาณนาฬิกาแทน ทำให้ชิป SSD1306 ไม่พบลูปจังหวะ Clock ที่ถูกต้อง และไม่สามารถถอดรหัสบิตข้อมูลได้เลย
* **หน้าจอจะติดหรือไม่:** **หน้าจอจะไม่ติด (มืดสนิท)** เนื่องจากชิปไม่สามารถรับคำสั่งเริ่มต้นระบบ (Magic Sequence) โดยเฉพาะคำสั่งเปิดวงจรปั๊มแรงดัน `0x8D, 0x14` (Charge Pump Enable) ส่งผลให้ไม่มีไฟแรงดันสูง 7.5V ไปเลี้ยงแผง OLED พิกเซลจึงไม่สามารถเปล่งแสงออกมาได้


3. เหตุใดการแก้ไขพิกัด $(x, y)$ บน `s_oled_buffer` จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง `oled_flush()`?
**คำตอบ:**
* เป็นเพราะระบบใช้สถาปัตยกรรมแบบ **Double Buffering (หรือ Framebuffer)**
* ตัวแปร `s_oled_buffer` เป็นเพียงก้อนหน่วยความจำแรม (SRAM) ขนาด 1,024 ไบต์ที่อยู่ภายในตัว **ESP32** เอง การสั่งฟังก์ชัน `oled_draw_pixel()` หรือเขียนข้อความ จึงเป็นแค่การคำนวณสลับบิต 0 และ 1 ในแรมของบอร์ดทดลอง ยังไม่มีการยิงสัญญาณออกไปภายนอก

* ภาพบนหน้าจอจะเปลี่ยนก็ต่อเมื่อเราเรียกฟังก์ชัน **`oled_flush()`** ซึ่งทำหน้าที่สั่งเปิดท่อส่งข้อมูล SPI เพื่อคัดลอกข้อมูล 1,024 ไบต์ทั้งหมดจากแรมของ ESP32 ข้ามสายสัญญาณไปเขียนทับลงในชิปหน่วยความจำ GDDRAM ของจอ SSD1306 จริง ๆ หน้าจอจึงจะอัปเดตภาพใหม่พร้อมกันในจังหวะเดียว


![alt text](image.png)

link : https://youtu.be/WcJvuuqqjso