...
**ตารางค่า Hex Dump ตัวอักษร 'H' (Column 30-34)**

ตัว 'H' เริ่มวาดที่ตำแหน่ง (x=30, y=4) ความสูงตัวอักษร 7 พิกเซล (y=4 ถึง y=10)
เนื่องจากแต่ละ Page ครอบคลุมแค่ 8 แถว (y) ตัวอักษรจึงคร่อมข้าม 2 pages:
- **Page 0** เก็บ y=4-7 (4 แถวบนของตัวอักษร)
- **Page 1** เก็บ y=8-10 (3 แถวล่างของตัวอักษร)

| Column | ค่าฟอนต์ (จากตาราง font5x7) | Page 0 (จาก Serial Monitor จริง) | Page 1 (คำนวณจากสูตร) |
|---|---|---|---|
| 30 | 0x7F = `0111 1111` | 0xF0 = `1111 0000` | 0x07 = `0000 0111` |
| 31 | 0x08 = `0000 1000` | 0x80 = `1000 0000` | 0x00 = `0000 0000` |
| 32 | 0x08 = `0000 1000` | 0x80 = `1000 0000` | 0x00 = `0000 0000` |
| 33 | 0x08 = `0000 1000` | 0x80 = `1000 0000` | 0x00 = `0000 0000` |
| 34 | 0x7F = `0111 1111` | 0xF0 = `1111 0000` | 0x07 = `0000 0111` |

*(หมายเหตุ: ค่า Page 0 คือค่าที่ dump ได้จริงจาก Serial Monitor ส่วนค่า Page 1 คำนวณจากสูตรเดียวกัน — หากต้องการยืนยันด้วยข้อมูลจริง ให้เพิ่มโค้ด dump ที่ offset +128 แล้วรันซ้ำเพื่อยืนยันค่า)*

**การถอดรหัสบิต (Bit-to-Pixel Decode)**

- Column 30 และ 34 (เส้นตั้งซ้าย-ขวาของตัว H): บิตติดต่อเนื่องตั้งแต่แถวบนสุดถึงล่างสุดของตัวอักษร (row 0-6 ทั้งหมด) → รวม 2 pages แล้วได้เส้นตรงยาวตลอดความสูงตัวอักษร ตรงกับรูปเส้นตั้งซ้าย-ขวาของตัว H บนจอจริง
- Column 31-33 (เส้นขวางกลางของตัว H): มีบิตติดแค่ตำแหน่งเดียวคือ row 3 (กึ่งกลางความสูงตัวอักษรพอดี) → ตรงกับรูปเส้นขวางกลางของตัว H บนจอจริง

**สรุปผล:** รูปแบบบิต 1 ในหน่วยความจำตรงกับรูปร่างตัวอักษร 'H' ที่ปรากฏบนจอจริง 100% ยืนยันว่าฟังก์ชัน `oled_draw_char` และสูตรคำนวณบิตใน `oled_draw_pixel` ทำงานถูกต้องสมบูรณ์
...
ตารางฟอนต์ 5x7 กำหนดให้ตัวอักษรแต่ละตัวมีความกว้าง 5 คอลัมน์ (5 ไบต์) และความสูง 7 แถว (7 บิตต่อไบต์) แต่ละไบต์ควบคุมพิกเซลในแนว**ตั้ง**ของคอลัมน์นั้นๆ กล่าวคือ 1 ไบต์ = 1 เส้นแนวตั้งของตัวอักษร โดยแต่ละบิตในไบต์แทนพิกเซล 1 จุดในแนวตั้งนั้น (bit 0 = แถวบนสุด, bit 6 = แถวล่างสุดของตัวอักษร) ส่วนการเรียงไบต์ 5 ไบต์ต่อกันคือการเรียงในแนว**นอน**จากซ้ายไปขวา จึงประกอบกันเป็นรูปตัวอักษรสมบูรณ์

**2. หากเราสลับสายไฟระหว่างขา D0 และ D1 จะเกิดผลอย่างไรกับสัญญาณ SPI และหน้าจอจะติดหรือไม่?**

D0 (SCK) คือสัญญาณนาฬิกาที่ใช้จับจังหวะการอ่านข้อมูล ส่วน D1 (MOSI) คือสายข้อมูลจริงที่ส่งบิตทีละบิต ถ้าสลับสายกัน ชิป SSD1306 จะได้รับสัญญาณนาฬิกาไปเข้าขาที่ควรจะเป็นข้อมูล และรับข้อมูลไปเข้าขาที่ควรจะเป็นนาฬิกา ทำให้จับจังหวะการอ่านบิตผิดพลาดทั้งหมด ผลคือจอจะไม่ติดเลย หรือถ้าติดก็จะแสดงภาพที่ผิดเพี้ยนสุ่มไม่มีความหมาย เนื่องจากคำสั่ง Magic Sequence ต่างๆ (Charge Pump, Display ON) ไม่ถูกส่งไปยังชิปอย่างถูกต้อง

**3. เหตุใดการแก้ไขพิกัด (x, y) บน s_oled_buffer จึงไม่ทำให้ภาพบนหน้าจอจริงเปลี่ยนทันที จนกว่าจะมีการเรียกคำสั่ง oled_flush()?**

เพราะ `s_oled_buffer` เป็นเพียงหน่วยความจำ (RAM) ภายในตัว ESP32 เท่านั้น ทำหน้าที่เป็น "กระดาษร่าง" หรือ Back Buffer สำหรับคำนวณภาพก่อน ไม่ได้เชื่อมต่อโดยตรงกับหน่วยความจำแสดงผล (GDDRAM) ที่อยู่ภายในชิป SSD1306 การแก้ไขค่าในบัฟเฟอร์นี้จึงเป็นแค่การเปลี่ยนแปลงข้อมูลในฝั่ง ESP32 เท่านั้น ต้องเรียก `oled_flush()` เพื่อส่งข้อมูลทั้งหมด 1,024 ไบต์ผ่านบัส SPI ไปเขียนทับหน่วยความจำจริงในชิป SSD1306 หน้าจอถึงจะปรากฏภาพตามที่ต้องการ

<img width="869" height="1884" alt="image" src="https://github.com/user-attachments/assets/4664cf16-54d8-47e4-ad02-36987727a6eb" />
