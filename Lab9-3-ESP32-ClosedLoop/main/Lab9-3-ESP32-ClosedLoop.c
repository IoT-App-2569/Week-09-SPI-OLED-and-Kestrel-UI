...
void render_multizone_ui(int raw_val, int percent, bool is_cloud, const char *msg) {
    ...
    // แก้ไขรหัสนักศึกษาให้ตรงกับของคุณ
    char header_str[32];
    snprintf(header_str, sizeof(header_str), "ESP32 | 67030120");
...
void app_main(void) {
    ...
    while (1) {
        ...
        // 1. อ่านค่า ADC จาก Potentiometer
        ESP_ERROR_CHECK(adc_oneshot_read(s_adc1_handle, POT_ADC1_CHAN, &raw_adc));

        // 2. ส่งค่าขึ้น Kestrel ทาง Serial Stream
        printf("ADC:%d,%" PRId64 "\n", raw_adc, now);
        fflush(stdout);

        // 3. อ่านคำสั่งตอบกลับจาก Kestrel ผ่าน UART
        uint8_t byte_in = 0;
        ...
        // 4. กลไก Hybrid Fallback (เกิน 1.5 วินาที ตัดเข้าสู่ Edge Mode)
