...
// Macro สำหรับแปลง Byte เป็น Binary 8 บิตเพื่อแสดงผลใน Terminal
...
// ฟังก์ชันล้างหน้าจอในแรม (เคลียร์เป็นสีดำสนิท)
void oled_clear(void)
{
    memset(s_oled_buffer, 0x00, sizeof(s_oled_buffer));
}

// ฟังก์ชันจุดหรือดับพิกเซลด้วยสูตรคณิตศาสตร์ระดับบิต
...
// ฟังก์ชันส่งถ่ายข้อมูล 1,024 ไบต์จากแรมขึ้นสู่หน้าจอจริง (Buffer Flush)
...
void app_main(void)
{
    ...
    oled_send_cmd(0x14); // 0x14 = Enable Charge Pump
    ...
    // แสดงข้อมูล 40 ไบต์แรกของ Page 0 เพื่อให้เห็นตัว 'H' (ซึ่งเริ่มที่คอลัมน์ 30 ถึง 34)
    ESP_LOGI("FORENSIC", "=== DUMPING FRAMEBUFFER PAGE 0 (First 40 Bytes) ===");
    for (int i = 0; i < 40; i++) {
