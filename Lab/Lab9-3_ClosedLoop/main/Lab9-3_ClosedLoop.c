...
void oled_clear(void) { memset(s_oled_buffer, 0x00, sizeof(s_oled_buffer)); }
...
void oled_fill_rect(int x, int y, int w, int h, bool color) {
    for (int i = 0; i < h; i++) oled_draw_line_h(x, y + i, w, color);
}
...
void init_hardware(void) {
    ...
    gpio_set_level(OLED_PIN_RES, 0); vTaskDelay(pdMS_TO_TICKS(15));
    gpio_set_level(OLED_PIN_RES, 1); vTaskDelay(pdMS_TO_TICKS(15));
...
void render_multizone_ui(int raw_val, int percent, bool is_cloud, const char *msg) {
    ...
    if (fill_w > 0) oled_fill_rect(12, 32, fill_w, 8, true);
    oled_draw_line_h(0, 48, 128, true); 

    // Zone 3: Mode & Notification
    if (is_cloud) oled_draw_string(2, 52, "CLOUD:", true);
    else          oled_draw_string(2, 52, "EDGE:", true);
...
void app_main(void) {
    ...
    while (1) {
        ...
        // อ่าน ADC และส่งผ่าน Serial
        ESP_ERROR_CHECK(adc_oneshot_read(s_adc1_handle, POT_ADC1_CHAN, &raw_adc));
        printf("ADC:%d,%" PRId64 "\n", raw_adc, now);
        fflush(stdout);

        // ตรวจสอบคำสั่งตอบกลับจาก Kestrel (Non-blocking UART)
        uint8_t byte_in = 0;
            ...
            if (byte_in == '\n' || byte_in == '\r') {
                if (rx_pos > 0) {
                    ...
                    if (sscanf(rx_line, "SET:%d:%31[^\r\n]", &incoming_percent, incoming_msg) == 2) {
                        ...
                        last_cloud_rx_time = now; 
                        is_cloud_mode = true;
                    }
                    rx_pos = 0; 
                }
            } else {
                if (rx_pos < sizeof(rx_line) - 1) rx_line[rx_pos++] = (char)byte_in;
            }
        }

        // Hybrid Fallback ตรวจสอบ Timeout (1.5 วินาที)
