...
- การคำนวณสเกลที่ Kestrel Server ช่วยลดภาระการประมวลผลของ ESP32 และทำให้สามารถอัปเดตสูตรคาลิเบรตจากศูนย์กลางได้ทันทีโดยไม่ต้องแฟลชเฟิร์มแวร์ใหม่

2. จากการทำ HTTP Forensics หากไม่มีการตรวจสอบเงื่อนไข `RawMax <= RawMin` ในโค้ด จะเกิด Exception ชนิดใดขึ้นในภาษา C# 
- และส่งผลต่อการทำงานของเซิร์ฟเวอร์อย่างไร?
    RawMax <= RawMin ทำให้สูตรคำนวณเกิดการหารด้วยศูนย์ ทำให้เกิดข้อผิดพลาด DivideByZeroException และเซิร์ฟเวอร์ตอบกลับด้วยรหัส 500 Internal Server Error
...
- เกิดจากการไม่ระบุ Header Content-Type: application/json ทำให้เซิร์ฟเวอร์ไม่ทราบวิธีแปลงข้อมูลดิบ (Deserialization) จึงปฏิเสธคำขอทันทีตามมาตรฐาน HTTP
