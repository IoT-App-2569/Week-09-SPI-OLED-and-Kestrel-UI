...
```
เพราะ ESP32 มีแรงและหน่วยความจำจำกัด ถ้าจะเปลี่ยนช่วงสเกลใหม่ต้องแก้โค้ดแล้ว flash ใหม่ทุกครั้ง แต่ทำที่ Kestrel แค่ยิง POST ก็เปลี่ยนได้ทันทีตอนรันอยู่ อีกอย่างถ้ามี ESP32 หลายตัว ก็แก้สูตรที่เซิร์ฟเวอร์ที่เดียวจบ ไม่ต้องไล่ flash ทีละตัว และหน้าที่ของ ESP32 ควรเป็นแค่ส่งค่าดิบที่ซื่อสัตย์ต่อเซนเซอร์ออกมา ส่วนการตีความค่าเป็นเรื่องของฝั่งเซิร์ฟเวอร์
```
...
```
ถ้า rawMax เท่ากับ rawMin พอดี ตัวหารจะเป็นศูนย์ แต่เพราะเป็น double ในภาษา C# จะไม่ throw exception มันคืน NaN หรือ Infinity ออกมาเงียบๆ เซิร์ฟเวอร์ไม่ล่มแต่ส่งค่าขยะไปให้ปลายทาง ซึ่งอันตรายกว่าล่มเพราะไม่มีใครรู้ตัว
ถ้า rawMax น้อยกว่า rawMin ตัวหารติดลบ สเกลจะกลับด้าน หมุน potentiometer ขึ้นแต่ค่าลดลง ทั้งสองกรณีไม่มี exception ให้จับเลย จึงต้องเช็คเองด้วย if ตั้งแต่แรก
```
...
```
เพราะ ASP.NET Core ผูก parameter ที่เป็น object อย่าง DisplayMessageRequest เข้ากับตัวอ่าน JSON ไว้ พอ request ไม่มี header บอกว่า body เป็น JSON ตัวอ่านก็ไม่รู้ว่าจะแปลง body ด้วยอะไร มันจึงปฏิเสธตั้งแต่ก่อนเข้า handler ของเรา รหัส 415 หมายถึงรูปแบบข้อมูลที่ส่งมาไม่รองรับ ต่างจาก 400 ที่แปลว่าเนื้อข้อมูลผิด สังเกตว่า response ของ 415 มี Content-Length 0 คือไม่มี body ตอบกลับเลย ยืนยันว่าโค้ดเราไม่เคยถูกเรียกทำงาน
```
