...
คำตอบ:
ควรคำนวณที่ฝั่ง Kestrel Server เพื่อให้ ESP32 ทำหน้าที่อ่านค่า Raw จากเซนเซอร์และส่งข้อมูลดิบไปยัง Server ส่วนการแปลงค่าและคำนวณสเกลให้ Server เป็นผู้จัดการ ทำให้สามารถเปลี่ยนสูตรหรือปรับช่วงการวัดได้โดยไม่ต้องแก้ไขและอัปโหลด Firmware ของ ESP32 ใหม่ อีกทั้งยังช่วยลดภาระการประมวลผลของไมโครคอนโทรลเลอร์ และทำให้สามารถนำข้อมูลดิบไปประมวลผลในรูปแบบอื่นได้ภายหลัง
...
คำตอบ:
หากไม่มีการตรวจสอบว่า RawMax <= RawMin ก่อนนำค่าไปคำนวณสเกล อาจทำให้เกิดการหารด้วยศูนย์เมื่อ RawMax มีค่าเท่ากับ RawMin ซึ่งในภาษา C# หากเป็นการคำนวณชนิดจำนวนเต็มจะเกิด DivideByZeroException ขึ้น ส่งผลให้การประมวลผล Request นั้นเกิดข้อผิดพลาด และหากไม่มีการจัดการ Exception ที่เหมาะสม อาจทำให้ Request ล้มเหลวและ Server ส่ง Error กลับไปยัง Client
...
คำตอบ:
เนื่องจาก Server ใช้ Header Content-Type เพื่อระบุว่าข้อมูลใน HTTP Request Body เป็นข้อมูลประเภทใด หาก API endpoint กำหนดให้รับข้อมูลในรูปแบบ JSON แต่ Request ไม่มี Content-Type: application/json Server จึงไม่สามารถยืนยันหรือเลือกตัวแปลงข้อมูล (JSON formatter/model binder) ที่ถูกต้องสำหรับ Request Body ได้ จึงปฏิเสธคำขอและส่งสถานะ 415 Unsupported Media Type กลับไปยัง Client
