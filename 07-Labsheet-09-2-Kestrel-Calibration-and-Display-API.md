...
## คำถามท้ายการทดลองเพื่อการประเมินผล (Review Questions)

### 1. เหตุใดการคำนวณสเกลเซนเซอร์จึงควรทำที่ฝั่ง Kestrel Server แทนที่จะคำนวณบนไมโครคอนโทรลเลอร์ ESP32 ตั้งแต่แรก?
**ตอบ** 
1. **ลดภาระทรัพยากรและการประมวลผลบนไมโครคอนโทรลเลอร์:** ไมโครคอนโทรลเลอร์ ESP32 มีหน้าที่หลักในการจัดการฮาร์ดแวร์แบบ Real-Time เช่น การอ่านค่าแอนะล็อก (ADC), การขับสัญญาณบัส SPI และการสแกนพิกเซลหน้าจอ การย้ายงานคำนวณ Floating-Point และการสเกลเชิงเส้นมาไว้ที่เซิร์ฟเวอร์ช่วยลดภาระซีพียูและหน่วยความจำของบอร์ด
2. **ความยืดหยุ่นในการปรับตั้งค่าแบบไดนามิก (Dynamic Calibration):** เมื่อเปลี่ยนเซนเซอร์หรือต้องการเปลี่ยนสเกล (เช่น แปลงเป็น %, RPM หรือ °C) สามารถอัปเดตผ่าน REST API บนเซิร์ฟเวอร์ได้ทันที โดยไม่ต้องแก้ไขซอร์สโค้ด C/C++ คอมไพล์ใหม่ หรือแฟลชเฟิร์มแวร์ลงบนบอร์ดไมโครคอนโทรลเลอร์ซ้ำ
3. **การประมวลผลแบบรวมศูนย์ (Centralized Processing & Consistency):** เซิร์ฟเวอร์ทำหน้าที่เป็นแหล่งข้อมูลความจริงหนึ่งเดียว (Single Source of Truth) สามารถเก็บประวัติการสอบเทียบ (Calibration History), ทำการกรองสัญญาณหรือชดเชยค่าแบบซับซ้อน และส่งข้อมูลสเกลมาตรฐานเดียวกันไปยังไคลเอนต์ปลายทางหลายตัวได้พร้อมกัน

### 2. จากการทำ HTTP Forensics หากไม่มีการตรวจสอบเงื่อนไข RawMax <= RawMin ในโค้ด จะเกิด Exception ชนิดใดขึ้นในภาษา C# และส่งผลต่อการทำงานของเซิร์ฟเวอร์อย่างไร?
**ตอบ** 
* **ชนิดของข้อผิดพลาด (Exception):** 
  * ในกรณีที่ใช้การคำนวณแบบทศนิยม `double` ตัวส่วน `(_settings.RawMax - _settings.RawMin)` เมื่อ `RawMax == RawMin` จะเท่ากับ `0.0` ซึ่งในมาตรฐาน IEEE 754 ของ C# จะไม่โยน Exception ออกมาทันที แต่จะได้ค่าผลลัพธ์เป็น **`Double.PositiveInfinity`**, **`Double.NegativeInfinity`** หรือ **`Double.NaN` (Not a Number)** ซึ่งทำให้ข้อมูล Telemetry เสียหายและไม่สามารถนำไปแปลงเป็น JSON หรือส่งต่อได้อย่างถูกต้อง
  * ในกรณีที่มีการคำนวณตัวส่วนด้วยชนิดข้อมูลจำนวนเต็ม (`int`) ก่อนการแคสต์ หรือสูตรเลขคณิตจำนวนเต็ม จะเกิดข้อผิดพลาด **`System.DivideByZeroException`**
* **ผลกระทบต่อเซิร์ฟเวอร์:** หากไม่มีการดักจับ (Unhandled Exception) จะทำให้ Request Pipeline ล้มเหลว เซิร์ฟเวอร์ Kestrel จะตอบกลับไคลเอนต์ด้วยรหัสสถานะ **`HTTP/1.1 500 Internal Server Error`** แม้เซิร์ฟเวอร์อาจไม่แครชทั้งกระบวนการ แต่ผู้ใช้งานจะไม่สามารถเรียกใช้งาน Endpoint ดังกล่าวได้และระบบขาดเสถียรภาพ

### 3. อธิบายสาเหตุทางเทคนิคว่าทำไมคำขอ HTTP POST ที่ไม่มี Header Content-Type: application/json จึงถูกปฏิเสธด้วยรหัสสถานะ 415 Unsupported Media Type?
**ตอบ** 
เพราะ Minimal API ของ ASP.NET Core ใช้กลไก **Content Negotiation** และกระบวนการ **Model Binding / JSON Deserialization** อัตโนมัติ โดยจะตรวจสอบค่าใน Header `Content-Type` ของ HTTP Request เพื่อเลือกตัวแปลงข้อมูล (Body Reader / Formatter) ที่เหมาะสม หากไคลเอนต์ส่ง Payload มาใน Request Body โดยไม่ระบุ `Content-Type: application/json` ตัว Kestrel จะไม่สามารถระบุชนิดของมีเดียได้ และปฏิเสธการประมวลผลข้อมูลตามข้อกำหนดของโปรโตคอล HTTP ด้วยรหัสสถานะ **`415 Unsupported Media Type`** เพื่อป้องกันข้อผิดพลาดจากการพยายามแปลงข้อมูลที่ไม่ทราบโครงสร้าง
